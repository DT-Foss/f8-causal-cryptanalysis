...
QUEUE_LOG="research/logs/chacha20_a434_a435_recovery_after_a431_v1.log"
...
A435_PROTOCOL="research/configs/chacha20_round20_w52_dual_axis_square_wavefront_recovery_a435_v1.json"
EXPECTED_A435_PROTOCOL_SHA256="a6616f0ce3ef8c92a67381e8d3c6acde649f0842d85ea7be9cc488e2182044cd"
A435_RESULT="research/results/v1/chacha20_round20_w52_dual_axis_square_wavefront_recovery_a435_v1.json"
A435_RUNNER="research/experiments/chacha20_round20_w52_dual_axis_square_wavefront_recovery_a435.py"
...
on_exit() { local status="$?"; record "A434/A435 supervisor exit status=$status"; }
trap on_exit EXIT
trap 'record "A434/A435 supervisor received signal"; exit 130' INT TERM HUP
...
progress_path() {
  local worker="$1"
  printf 'research/results/v1/chacha20_round20_w52_dual_axis_square_wavefront_recovery_a435_worker_%s_progress_v1.json\n' "$worker"
}
...
worker_active() {
  local worker="$1"
  pgrep -f "chacha20_round20_w52_dual_axis_square_wavefront_recovery_a435.py --recover-worker ${worker}([[:space:]]|$)" \
    >/dev/null 2>&1
}
...
launch_worker() {
  ...
  local log="research/logs/chacha20_round20_w52_a435_worker_${worker}_recovery.log"
  record "launch A435 worker=$worker protocol_sha256=$EXPECTED_A435_PROTOCOL_SHA256 qualification_sha256=$qualification_sha256"
  caffeinate -dimsu nice -n 10 \
    .venv/bin/python "$A435_RUNNER" \
    --recover-worker "$worker" \
    --expected-protocol-sha256 "$EXPECTED_A435_PROTOCOL_SHA256" \
    --expected-a434-qualification-sha256 "$qualification_sha256" \
    >>"$log" 2>&1 &
}

record "A434/A435 supervisor start max_active=$MAX_ACTIVE"
...
observed_protocol_sha256="$(shasum -a 256 "$A435_PROTOCOL" | awk '{print $1}')"
if [[ "$observed_protocol_sha256" != "$EXPECTED_A435_PROTOCOL_SHA256" ]]; then
  record "A435 protocol hash mismatch expected=$EXPECTED_A435_PROTOCOL_SHA256 observed=$observed_protocol_sha256"
  exit 1
fi
...
.venv/bin/python "$A435_RUNNER" --analyze >/dev/null
record "exclusive eight-slot A435 launch window acquired qualification_sha256=$qualification_sha256"

while [[ ! -f "$A435_RESULT" ]]; do
  launched=no
  ...
  if [[ "$launched" == no ]]; then
    ...
    if (( exhausted == ${#WORKERS[@]} )); then
      record "all eight A435 schedules exhausted without a confirmed model"
      exit 1
    fi
    sleep "$POLL_SECONDS"
  fi
...
result_sha256="$(shasum -a 256 "$A435_RESULT" | awk '{print $1}')"
record "A435 result complete sha256=$result_sha256"
