...
RUNNER="research/experiments/chacha20_round20_w50_knownkey_nested_fisher_reader_a408.py"
IMPLEMENTATION="research/configs/chacha20_round20_w50_knownkey_nested_fisher_reader_a408_implementation_v1.json"
...
RESULT="research/results/v1/chacha20_round20_w50_knownkey_nested_fisher_reader_a408_v1.json"
TEST="tests/test_chacha20_round20_w50_knownkey_nested_fisher_reader_a408.py"

if [[ ! -f "$IMPLEMENTATION" ]]; then
  "$PYTHON" "$RUNNER" --freeze-implementation
fi

if [[ -f "$A401_SELECTION" && -f "$A401_RESULT" && -f "$A406_RESULT" && ! -f "$RESULT" ]]; then
