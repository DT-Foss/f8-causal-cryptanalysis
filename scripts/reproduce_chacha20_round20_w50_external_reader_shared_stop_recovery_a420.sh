#!/usr/bin/env bash
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

PYTHON="${PYTHON:-.venv/bin/python}"
RUNNER="research/experiments/chacha20_round20_w50_external_reader_shared_stop_recovery_a420.py"
TEST="tests/test_chacha20_round20_w50_external_reader_shared_stop_recovery_a420.py"
IMPLEMENTATION="research/configs/chacha20_round20_w50_external_reader_shared_stop_recovery_a420_implementation_v1.json"
PROTOCOL="research/configs/chacha20_round20_w50_external_reader_shared_stop_recovery_a420_v1.json"
A418_RESULT="research/results/v1/chacha20_round20_w50_selection_calibrated_portfolio_repair_a418_v1.json"
A419_RESULT="research/results/v1/chacha20_round20_w50_majority_polarity_portfolio_a419_v1.json"

run_tests() {
  PYTHONWARNINGS=error "$PYTHON" -m pytest -q "$TEST"
}

case "${1:---analyze}" in
  --freeze-implementation)
    run_tests
    "$PYTHON" "$RUNNER" --freeze-implementation
    ;;
  --freeze-protocol)
    run_tests
    [[ -f "$IMPLEMENTATION" && -f "$A418_RESULT" && -f "$A419_RESULT" ]]
    implementation_sha256="$(shasum -a 256 "$IMPLEMENTATION" | awk '{print $1}')"
    a418_sha256="$(shasum -a 256 "$A418_RESULT" | awk '{print $1}')"
    a419_sha256="$(shasum -a 256 "$A419_RESULT" | awk '{print $1}')"
    "$PYTHON" "$RUNNER" \
      --freeze-protocol \
      --expected-implementation-sha256 "$implementation_sha256" \
      --expected-a418-result-sha256 "$a418_sha256" \
      --expected-a419-result-sha256 "$a419_sha256"
    ;;
  --recover-worker)
    [[ $# -eq 2 ]]
    run_tests
    [[ -f "$PROTOCOL" ]]
    protocol_sha256="$(shasum -a 256 "$PROTOCOL" | awk '{print $1}')"
    "$PYTHON" "$RUNNER" \
      --recover-worker "$2" \
      --expected-protocol-sha256 "$protocol_sha256"
    ;;
  --analyze)
    run_tests
    "$PYTHON" "$RUNNER" --analyze
    ;;
  *)
    printf 'usage: %s [--freeze-implementation|--freeze-protocol|--recover-worker INDEX|--analyze]\n' "$0" >&2
    exit 2
    ;;
esac
