...
RUNNER="research/experiments/chacha20_round20_w50_nested_prototype_recovery_a411.py"
IMPLEMENTATION="research/configs/chacha20_round20_w50_nested_prototype_recovery_a411_implementation_v1.json"
PROTOCOL="research/configs/chacha20_round20_w50_nested_prototype_recovery_a411_v1.json"
...
TEST="tests/test_chacha20_round20_w50_nested_prototype_recovery_a411.py"
...
if [[ -f "$A402_RESULT" && -f "$A404_RESULT" && -f "$A406_RESULT" && -f "$A408_RESULT" && -f "$A410_RESULT" && ! -f "$PROTOCOL" ]]; then
  QUALIFIED="$($PYTHON -c 'import json,sys; print(str(json.load(open(sys.argv[1]))["comparison"]["qualified"]).lower())' "$A410_RESULT")"
  if [[ "$QUALIFIED" == "true" ]]; then
    ...
    A410_RESULT_SHA256="$(shasum -a 256 "$A410_RESULT" | awk '{print $1}')"
    "$PYTHON" "$RUNNER" --freeze-protocol \
      --expected-implementation-sha256 "$IMPLEMENTATION_SHA256" \
      --expected-a410-result-sha256 "$A410_RESULT_SHA256" \
...
if [[ "$RECOVER" == "true" ]]; then
  if [[ ! -f "$PROTOCOL" ]]; then
    printf 'A411 recovery requires a qualified A410 result and frozen protocol\n' >&2
    exit 1
  fi
  ...
  if [[ "$EXECUTION_ENABLED" != "true" ]]; then
    printf 'A411 protocol is equivalence-only because a prior learned order is identical\n' >&2
    exit 1
  fi
