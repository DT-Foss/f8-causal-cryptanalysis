...
RUNNER="research/experiments/chacha20_round20_w50_weighted_reader_recovery_a407.py"
IMPLEMENTATION="research/configs/chacha20_round20_w50_weighted_reader_recovery_a407_implementation_v1.json"
PROTOCOL="research/configs/chacha20_round20_w50_weighted_reader_recovery_a407_v1.json"
...
TEST="tests/test_chacha20_round20_w50_weighted_reader_recovery_a407.py"
...
if [[ -f "$A402_RESULT" && -f "$A404_RESULT" && -f "$A406_RESULT" && ! -f "$PROTOCOL" ]]; then
  QUALIFIED="$($PYTHON -c 'import json,sys; print(str(json.load(open(sys.argv[1]))["leaveoneout"]["qualified"]).lower())' "$A406_RESULT")"
...
if [[ "$RECOVER" == "true" ]]; then
  if [[ ! -f "$PROTOCOL" ]]; then
    printf 'A407 recovery requires a qualified A406 result and frozen protocol\n' >&2
    exit 1
  fi
  ...
  if [[ "$EXECUTION_ENABLED" != "true" ]]; then
    printf 'A407 protocol is equivalence-only because a prior learned order is identical\n' >&2
    exit 1
  fi
