...
Logical complexity remains exactly `2^k`. Candidate packing changes executable
state evaluations and memory traffic, not the exponent. Checkpoints are written
atomically; resume continues at the next unprocessed pack and never accepts a
cached final conclusion.
