...
Arguments parse_arguments(int argc, char **argv) {
  ...
  if (result.load_only) {
    if (!result.cell_order.empty() || result.seconds != 0.0)
      throw std::runtime_error(
          "load-only forbids cell-order and positive solve seconds");
    return result;
  }
  if (result.seconds <= 0.0)
    throw std::runtime_error("seconds are required outside load-only mode");
...
int main(int argc, char **argv) {
  try {
    ...
    if (arguments.load_only) {
      std::cout << "R20_XFER_LOAD {\"signature\":\""
    ...
    for (std::size_t cell_index = 0;
      ...
      std::cout << "R20_XFER_RESULT {\"mode\":\"" << arguments.mode
                << "\",\"prefix8\":\"" << prefix8
    ...
    std::cout << "R20_XFER_SUMMARY {\"signature\":\""
              << CaDiCaL::Solver::signature() << "\",\"version\":\""
              << CaDiCaL::Solver::version() << "\",\"mode\":\""
              << arguments.mode << "\",\"variables\":" << variables
              << ",\"cells\":256,\"sat\":" << sat
    ...
    std::cerr << "R20_XFER_ERROR " << error.what() << '\n';
    return 2;
  }
}
