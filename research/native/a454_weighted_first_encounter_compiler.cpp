...
constexpr std::array<unsigned char, 16> kMagic = {
    'A', '4', '5', '4', 'R', 'A', 'N', 'K', 'V', '1', 0, 0, 0, 0, 0, 0};
...
std::uint16_t read_u16be(std::istream& input) {
  ...
  if (!input) {
    throw std::runtime_error("A454 component input ended early");
  }
...
std::array<std::uint16_t, kAxisCells> read_inverse_permutation(
  ...
  for (std::uint32_t position = 0; position < kAxisCells; ++position) {
    ...
    if (value >= kAxisCells || seen[value]) {
      throw std::runtime_error("A454 component order is not a permutation");
    }
    seen[value] = true;
    inverse[value] = static_cast<std::uint16_t>(position);
  }
  if (!std::all_of(seen.begin(), seen.end(), [](bool value) { return value; })) {
    throw std::runtime_error("A454 component order cover is incomplete");
  }
  return inverse;
}
...
std::array<ComponentOrder, kComponents> read_components(const std::string& path) {
  ...
  if (!input) {
    throw std::runtime_error("A454 cannot open component input");
  }
  std::array<unsigned char, 16> magic{};
  input.read(reinterpret_cast<char*>(magic.data()), magic.size());
  if (!input || magic != kMagic) {
    throw std::runtime_error("A454 component input magic differs");
  }
  ...
  if (input.peek() != std::char_traits<char>::eof()) {
    throw std::runtime_error("A454 component input has trailing bytes");
  }
  return components;
}
...
std::uint32_t square_rank(const ComponentOrder& component,
  ...
  if (zero_based >= kPairCells) {
    throw std::runtime_error("A454 component rank is outside the pair domain");
  }
...
std::size_t component_index(char symbol) {
  ...
  if (found == kSymbols.end()) {
    throw std::runtime_error("A454 pattern contains an unknown symbol");
  }
  return static_cast<std::size_t>(found - kSymbols.begin());
}

Pattern parse_pattern(const std::string& text) {
  if (text.size() < 3u || text.size() > 6u) {
    throw std::runtime_error("A454 pattern period must be in [3,6]");
  }
  ...
  if (std::any_of(pattern.occurrence_slots.begin(),
    ...
    throw std::runtime_error("A454 pattern must contain H, B, and O");
  }
  return pattern;
}
...
std::uint64_t proposal_key(const Pattern& pattern, std::size_t component,
  ...
  if (one_based_rank == 0u) {
    throw std::runtime_error("A454 proposal rank must be one based");
  }
...
void write_schedule(const std::string& path, const std::vector<Job>& jobs) {
  ...
  if (!output) {
    throw std::runtime_error("A454 cannot open output artifact");
  }
  std::uint64_t previous_key = 0;
  bool first = true;
  for (const Job& job : jobs) {
    if (!first && job.first_key <= previous_key) {
      throw std::runtime_error("A454 first-encounter keys are not unique");
    }
    first = false;
  ...
  output.flush();
  if (!output) {
    throw std::runtime_error("A454 output artifact write failed");
  }
}

void self_test() {
  const Pattern pattern = parse_pattern("HHBO");
  if (proposal_key(pattern, 0u, 1u) != 0u ||
      proposal_key(pattern, 0u, 2u) != 1u ||
      proposal_key(pattern, 0u, 3u) != 4u ||
      proposal_key(pattern, 1u, 1u) != 2u ||
      proposal_key(pattern, 1u, 2u) != 6u ||
      proposal_key(pattern, 2u, 1u) != 3u) {
    throw std::runtime_error("A454 weighted proposal-key self-test failed");
  }
  ...
  if (square_rank(identity, 0u) != 1u ||
    ...
    throw std::runtime_error("A454 square-rank self-test failed");
  }
  ...
  if (!earlier(first, second) || earlier(second, first)) {
    throw std::runtime_error("A454 ordering self-test failed");
  }
}
...
int main(int argc, char** argv) {
  try {
    if (argc == 2 && std::string(argv[1]) == "--self-test") {
      self_test();
      std::cout << "A454 weighted compiler self-test passed\n";
      return 0;
    }
    if (argc != 4) {
    ...
    std::cerr << "A454 building weighted first-encounter keys for pattern "
              << pattern.text << '\n';
    auto jobs = build_jobs(components, pattern);
    std::cerr << "A454 sorting " << jobs.size() << " cells\n";
    std::sort(jobs.begin(), jobs.end(), earlier);
    std::cerr << "A454 writing complete pair permutation\n";
    write_schedule(argv[2], jobs);
    std::cerr << "A454 complete\n";
    return 0;
