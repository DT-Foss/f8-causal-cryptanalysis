...
constexpr std::array<unsigned char, 16> kMagic = {
    'A', '4', '5', '8', 'R', 'A', 'N', 'K', 'V', '1', 0, 0, 0, 0, 0, 0};
...
std::uint16_t read_u16be(std::istream& input) {
  ...
  if (!input) {
    throw std::runtime_error("A458 component input ended early");
  }
...
std::array<std::uint16_t, kAxisCells> read_inverse_permutation(
  ...
  for (std::uint32_t position = 0; position < kAxisCells; ++position) {
    ...
    if (value >= kAxisCells || seen[value]) {
      throw std::runtime_error("A458 component order is not a permutation");
    }
    seen[value] = true;
    inverse[value] = static_cast<std::uint16_t>(position);
  }
  if (!std::all_of(seen.begin(), seen.end(), [](bool value) { return value; })) {
    throw std::runtime_error("A458 component order cover is incomplete");
  }
  return inverse;
}
...
std::array<ComponentOrder, kComponents> read_components(const std::string& path) {
  ...
  if (!input) {
    throw std::runtime_error("A458 cannot open component input");
  }
  std::array<unsigned char, 16> magic{};
  input.read(reinterpret_cast<char*>(magic.data()), magic.size());
  if (!input || magic != kMagic) {
    throw std::runtime_error("A458 component input magic differs");
  }
  ...
  if (input.peek() != std::char_traits<char>::eof()) {
    throw std::runtime_error("A458 component input has trailing bytes");
  }
  return components;
}
...
std::uint32_t square_rank(const ComponentOrder& component,
  ...
  if (zero_based >= kPairCells) {
    throw std::runtime_error("A458 component rank is outside the pair domain");
  }
...
std::size_t component_index(char symbol) {
  ...
  if (found == kSymbols.end()) {
    throw std::runtime_error("A458 pattern contains an unknown symbol");
  }
  return static_cast<std::size_t>(found - kSymbols.begin());
}

Pattern parse_pattern(const std::string& text) {
  if (text.size() < 2u || text.size() > 31u) {
    throw std::runtime_error("A458 pattern period must be in [2,31]");
  }
  ...
  if (pattern.occurrence_slots[component_index('H')].empty() ||
      pattern.occurrence_slots[component_index('O')].empty()) {
    throw std::runtime_error("A458 pattern must contain H and O");
  }
  return pattern;
}
...
std::uint64_t proposal_key(const Pattern& pattern, std::size_t component,
  ...
  if (one_based_rank == 0u) {
    throw std::runtime_error("A458 proposal rank must be one based");
  }
  const auto& slots = pattern.occurrence_slots[component];
  if (slots.empty()) {
    throw std::runtime_error("A458 proposal requested for inactive component");
  }
...
std::vector<Job> build_jobs(
  ...
  for (std::uint32_t canonical_id = 0; canonical_id < kPairCells;
    ...
    for (std::size_t component = 0; component < kComponents; ++component) {
      if (pattern.occurrence_slots[component].empty()) {
        continue;
      }
      first_key = std::min(
          first_key,
...
void write_schedule(const std::string& path, const std::vector<Job>& jobs) {
  ...
  if (!output) {
    throw std::runtime_error("A458 cannot open output artifact");
  }
  std::uint64_t previous_key = 0;
  bool first = true;
  for (const Job& job : jobs) {
    if (!first && job.first_key <= previous_key) {
      throw std::runtime_error("A458 first-encounter keys are not unique");
    }
    first = false;
  ...
  output.flush();
  if (!output) {
    throw std::runtime_error("A458 output artifact write failed");
  }
}
...
void exhaustive_small_semantics_test() {
  ...
  std::array<char, 3> three_symbols = {'B', 'H', 'O'};
  do {
    patterns.emplace_back(three_symbols.begin(), three_symbols.end());
  } while (std::next_permutation(three_symbols.begin(), three_symbols.end()));
  std::array<char, 2> two_symbols = {'H', 'O'};
  do {
    patterns.emplace_back(two_symbols.begin(), two_symbols.end());
  } while (std::next_permutation(two_symbols.begin(), two_symbols.end()));
  for (const std::string& text : patterns) {
    const Pattern pattern = parse_pattern(text);
          ...
          for (std::size_t rank = 0; rank < kSmallCells; ++rank) {
            if (calculated[rank].canonical_id != simulated[rank]) {
              throw std::runtime_error(
                  "A458 exhaustive small first-encounter semantics failed");
            }
          }
...
void self_test() {
  ...
  if (proposal_key(pattern, 0u, 1u) != 4u ||
    ...
    throw std::runtime_error("A458 frequency-ray proposal-key self-test failed");
  }
  const Pattern endpoint = parse_pattern("OOOHHH");
  if (proposal_key(endpoint, 0u, 1u) != 3u ||
      proposal_key(endpoint, 0u, 4u) != 9u ||
      proposal_key(endpoint, 2u, 1u) != 0u ||
      proposal_key(endpoint, 2u, 4u) != 6u) {
    throw std::runtime_error("A458 B0 endpoint proposal-key self-test failed");
  }
  ...
  if (square_rank(identity, 0u) != 1u ||
    ...
    throw std::runtime_error("A458 square-rank self-test failed");
  }
  ...
  if (!earlier(first, second) || earlier(second, first)) {
    throw std::runtime_error("A458 ordering self-test failed");
  }
...
int main(int argc, char** argv) {
  try {
    if (argc == 2 && std::string(argv[1]) == "--self-test") {
      self_test();
      std::cout << "A458 frequency-ray compiler self-test passed\n";
      return 0;
    }
    if (argc != 4) {
    ...
    std::cerr << "A458 building frequency-ray keys for pattern "
              << pattern.text << '\n';
    auto jobs = build_jobs(components, pattern);
    std::cerr << "A458 sorting " << jobs.size() << " cells\n";
    std::sort(jobs.begin(), jobs.end(), earlier);
    std::cerr << "A458 writing complete pair permutation\n";
    write_schedule(argv[2], jobs);
    std::cerr << "A458 complete\n";
    return 0;
