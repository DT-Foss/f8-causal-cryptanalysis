...
Arguments parse_arguments(int argc, char **argv) {
  ...
  if (result.assumption_vars.size() != 8)
    throw std::runtime_error("exactly eight assumption variables are required");
  if (result.model_vars.size() != 20)
    throw std::runtime_error("exactly twenty model variables are required");
  if (std::set<int>(result.assumption_vars.begin(),
                    result.assumption_vars.end())
          .size() != 8)
    throw std::runtime_error("assumption variables must be distinct");
  if (std::set<int>(result.model_vars.begin(), result.model_vars.end()).size() !=
      20)
...
int main(int argc, char **argv) {
  try {
    ...
    for (std::size_t cell_index = 0; cell_index < arguments.cell_order.size();
         ++cell_index) {
      const std::string &prefix8 = arguments.cell_order[cell_index];
      std::vector<int> assumptions;
      for (std::size_t bit = 0; bit < 8; ++bit) {
        const int variable = arguments.assumption_vars[bit];
        assumptions.push_back(prefix8[bit] == '1' ? variable : -variable);
      }
      ...
      std::cout << "A211_RESULT {\"mode\":\"" << arguments.mode
                << "\",\"prefix8\":\"" << prefix8
