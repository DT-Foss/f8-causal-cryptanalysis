#!/usr/bin/env python3
"""A456: extend A454 along a structured no-refit H:O:B frequency ray."""
...
STEM = "chacha20_round20_w52_no_refit_frequency_ray_portfolio_a456"
...
NATIVE_SOURCE = RESEARCH / "native/a456_frequency_ray_compiler.cpp"
NATIVE_EXECUTABLE = RESEARCH / "bin/a456_frequency_ray_compiler"
...
ATTEMPT_ID = "A456"
DESIGN_SHA256 = "a1a2529fbb9280d4f031f7fb816eb3aa85a64671406c38bbd60b4c80056a800f"
...
NATIVE_SOURCE_SHA256 = "c27a487cc3fefb9610eea09148513a715e7673ea7018f71a82f3a00d1395127c"
NATIVE_EXECUTABLE_SHA256 = (
    "d5c66b485cfa0239099c38446dde0438c75edf60c7a8642f7ab3aa1cb026fd7f"
)
...
M_VALUES = (3, 4, 5, 6)
EXPECTED_BY_M = {3: 140, 4: 126, 5: 352, 6: 260}
EXPECTED_ORBITS_BY_M = {3: 20, 4: 14, 5: 32, 6: 20}
EXPECTED_PATTERNS = 878
EXPECTED_ORBITS = 86
A454_CONTROL_PATTERN = "BOOHH"
A454_CONTROL_AGGREGATE = 0.47170094050966593
A454_CONTROL_MINIMUM = 0.1607590806312036
AXIS_CELLS = 1 << 12
PAIR_CELLS = 1 << 24
CHUNK = 1 << 20
TOP_KS = (16, 64, 256, 1024, 65536, 1048576)
MAGIC = b"A456RANKV1" + bytes(6)
DOTCAUSAL_SRC = Path(
...
def load_module(path: Path, name: str) -> Any:
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot import A456 dependency {path}")
...
A454 = load_module(A454_RUNNER, "a456_a454")
A448 = A454.A448
...
def topology_bases(m: int) -> set[str]:
    if m not in M_VALUES:
        raise ValueError("A456 frequency-ray m differs")
    bases = {"O" * m + "H" * m, "OH" * m}
    low = m // 2
    high = m - low
    for o_runs in set(itertools.permutations((low, high))):
        for h_runs in set(itertools.permutations((low, high))):
            bases.add(
                "O" * o_runs[0]
                + "H" * h_runs[0]
                + "O" * o_runs[1]
...
def enumerate_patterns_by_m() -> dict[int, tuple[str, ...]]:
    output: dict[int, tuple[str, ...]] = {}
    for m in M_VALUES:
        ho_words: set[str] = set()
        for base in topology_bases(m):
            ho_words.update(rotations(base))
        patterns = {
            word[:slot] + "B" + word[slot:]
            for word in ho_words
            for slot in range(len(word) + 1)
        }
        ordered = tuple(sorted(patterns))
        if len(ordered) != EXPECTED_BY_M[m]:
            raise RuntimeError(f"A456 m={m} pattern count differs")
        if any(
            row.count("H") != m
            or row.count("B") != 1
            or row.count("O") != m
            ...
            raise RuntimeError(f"A456 m={m} symbol counts differ")
        output[m] = ordered
    if sum(map(len, output.values())) != EXPECTED_PATTERNS:
        raise RuntimeError("A456 total candidate count differs")
    return output


PATTERNS_BY_M = enumerate_patterns_by_m()
PATTERNS = tuple(
    pattern for m in M_VALUES for pattern in PATTERNS_BY_M[m]
)
PATTERN_M = {
    pattern: m for m in M_VALUES for pattern in PATTERNS_BY_M[m]
}
...
if len(ORBITS) != EXPECTED_ORBITS:
    raise RuntimeError("A456 cyclic orbit count differs")
for m in M_VALUES:
    count = sum(PATTERN_M[orbit] == m for orbit in ORBITS)
    if count != EXPECTED_ORBITS_BY_M[m]:
        raise RuntimeError(f"A456 m={m} orbit count differs")
...
def load_design() -> dict[str, Any]:
    ...
    if (
        value.get("schema")
        != "chacha20-round20-w52-no-refit-frequency-ray-portfolio-a456-design-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or source.get("A454_result_sha256") != A454_RESULT_SHA256
        or source.get("A454_result_commitment_sha256")
        != A454_RESULT_COMMITMENT_SHA256
        or source.get("A454_calibration_sha256") != A454_CALIBRATION_SHA256
        or source.get("A454_selected_pattern") != A454_CONTROL_PATTERN
        or source.get("component_symbol_map") != COMPONENTS
        or tuple(family.get("frequency_ray_m_values", [])) != M_VALUES
        or family.get("expected_unique_patterns") != EXPECTED_PATTERNS
        or family.get("expected_cyclic_orbits") != EXPECTED_ORBITS
        or family.get("m3_family_is_exhaustive_over_all_H3_B1_O3_patterns")
        is not True
        ...
        or selection.get("A454_BOOHH_is_frozen_external_control_not_a_candidate")
        is not True
        ...
        or boundary.get("A456_candidate_results_seen_before_design") is not False
        or boundary.get("A456_selected_pattern_seen_before_design") is not False
        or boundary.get("A456_W52_pair_stream_seen_before_design") is not False
        or boundary.get(
            "A426_A438_A440_A443_A450_A452_A455_secret_result_stop_or_worker_progress_read"
        )
        is not False
        ...
        raise RuntimeError("A456 frozen design semantics differ")
...
def load_sources() -> tuple[
    ...
    a454 = A454.load_result(A454_RESULT_SHA256)
    a448, a449, a451, a453 = A454.load_sources()
    if (
        a454.get("result_commitment_sha256")
        != A454_RESULT_COMMITMENT_SHA256
        or a454.get("calibration", {}).get("calibration_sha256")
        != A454_CALIBRATION_SHA256
        or a454.get("calibration", {}).get("selected_pattern")
        != A454_CONTROL_PATTERN
        or a454.get("W52_target_labels_used") != 0
        or a454.get("feature_refits") != 0
        or a454.get("model_refits") != 0
        or a454.get("W52_candidate_assignments_executed") != 0
    ):
        raise RuntimeError("A456 A454 source semantics differ")
    return a454, a448, a449, a451, a453


def freeze_implementation() -> dict[str, Any]:
    downstream = (IMPLEMENTATION, RESULT, ARTIFACT, COMPONENT_INPUT, CAUSAL, REPORT)
    if any(path.exists() for path in downstream):
        raise FileExistsError("A456 implementation or downstream artifact exists")
    design = load_design()
    load_sources()
    for path in (TEST, REPRO, NATIVE_SOURCE, NATIVE_EXECUTABLE):
        if not path.exists():
            raise FileNotFoundError(f"A456 required artifact absent: {path}")
    anchor(NATIVE_SOURCE, NATIVE_SOURCE_SHA256)
    anchor(NATIVE_EXECUTABLE, NATIVE_EXECUTABLE_SHA256)
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w52-no-refit-frequency-ray-portfolio-a456-implementation-v1",
        "attempt_id": ATTEMPT_ID,
        "commitment_state": "complete_structured_frequency_ray_evaluator_and_period31_W52_compiler_frozen_before_any_A456_candidate_value_or_selected_pattern",
        ...
        "patterns_by_m": {str(m): len(PATTERNS_BY_M[m]) for m in M_VALUES},
        "orbits_by_m": {
            str(m): sum(PATTERN_M[orbit] == m for orbit in ORBITS)
            for m in M_VALUES
        },
        "selection_objective": design["selection_contract"][
            "lexicographic_objective"
        ],
        "A456_candidate_results_seen_at_freeze": False,
        "A456_selected_pattern_seen_at_freeze": False,
        "A456_W52_pair_stream_seen_at_freeze": False,
...
def load_implementation(expected_sha256: str) -> dict[str, Any]:
    ...
    if (
        value.get("schema")
        != "chacha20-round20-w52-no-refit-frequency-ray-portfolio-a456-implementation-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        ...
        or value.get("A456_candidate_results_seen_at_freeze") is not False
        or value.get("A456_selected_pattern_seen_at_freeze") is not False
        or value.get("A456_W52_pair_stream_seen_at_freeze") is not False
        ...
        raise RuntimeError("A456 implementation semantics differ")
    ...
    if value.get("implementation_commitment_sha256") != canonical_sha256(unsigned):
        raise RuntimeError("A456 implementation commitment differs")
    return value


def pattern_complexity(pattern: str) -> dict[str, Any]:
    row = A454.pattern_complexity(pattern)
    row["m"] = PATTERN_M[pattern]
    row["cyclic_orbit"] = PATTERN_ORBIT[pattern]
    row["cyclic_HO_transitions"] = cyclic_ho_transitions(pattern)
    return row
...
def calibration_field(
    ...
    return A454.calibration_field(fields, targets, pattern)
...
def calibration_result() -> dict[str, Any]:
    a454, _a448, _a449, _a451, _a453 = load_sources()
    ...
    for index, pattern in enumerate(PATTERNS, start=1):
        ...
        if index % 25 == 0 or index == len(PATTERNS):
            print(
                f"A456 calibrated {index}/{len(PATTERNS)} structured schedules",
                file=sys.stderr,
                flush=True,
            )
    ...
    if not eligible:
        raise RuntimeError("A456 candidate family has no eight-block-positive schedule")
    ranked = sorted(
        eligible,
    ...
    a454_stats = a454["calibration"]["selected_remaining96_full_statistics"]
    a454_complete = a454["calibration"]["selected_complete128_report_only"]
    strict_improvement = bool(
        selected_stats["minimum_fixed_block_bit_gain"]
        > a454_stats["minimum_fixed_block_bit_gain"]
        or (
            selected_stats["minimum_fixed_block_bit_gain"]
            == a454_stats["minimum_fixed_block_bit_gain"]
            and selected_stats["aggregate_bit_gain"]
            > a454_stats["aggregate_bit_gain"]
        )
    )
    per_m = {}
    for m in M_VALUES:
        subset = [row for row in rows if row["complexity"]["m"] == m]
        subset_eligible = [
            row
            for row in subset
            if row["remaining96_A447_fixed_model_no_refit"][
        ...
        per_m[str(m)] = {
            ...
            "best_statistics": best[
                "remaining96_A447_fixed_model_no_refit"
            ],
            "best_orbit_statistics": orbit_rows[
                best["complexity"]["cyclic_orbit"]
            ],
        }
    result: dict[str, Any] = {
        ...
        "A454_BOOHH_external_control_remaining96": a454_stats,
        "A454_BOOHH_external_control_complete128_report_only": a454_complete,
        "selected_delta_over_A454_BOOHH": {
...
def write_component_input(a449: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for symbol in SYMBOLS:
        ...
        if (
            ...
            raise RuntimeError(f"A456 W52 component {name} order differs")
    ...
    if len(payload) != expected_bytes:
        raise RuntimeError("A456 component input size differs")
...
def compile_W52_stream(
    ...
    if temporary.stat().st_size != PAIR_CELLS * 4:
        temporary.unlink(missing_ok=True)
        raise RuntimeError("A456 native compiler artifact size differs")
    ...
    bounds = A454.exact_W52_bounds(
        fused_rank, component_ranks, selected_pattern
    )
    first_key = np.minimum.reduce(
        [
            A454.proposal_key_vector(
    ...
    if np.any(sorted_keys[1:] <= sorted_keys[:-1]):
        raise RuntimeError("A456 complete W52 first keys are not unique")
    if np.any(
        fused_rank[expected_order]
        != np.arange(1, PAIR_CELLS + 1, dtype=np.uint32)
    ):
        raise RuntimeError("A456 native stream differs from Python first-key order")
    ...
    a454_rank, a454_artifact = artifact_rank_vector(A454_PAIR_STREAM)
    comparison_a454 = compare_ranks(fused_rank, a454_rank)
    del a454_rank
    geometry = {
        "comparison_to_A454_BOOHH_stream": comparison_a454,
        "component_comparisons": {
            symbol: compare_ranks(fused_rank, component_ranks[symbol])
            for symbol in SYMBOLS
        },
        "A454_source_artifact": a454_artifact,
...
def build_causal(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    writer = CausalWriter(api_id="a456ray")
    writer._rules = []
    writer.add_rule(
        name="frequency_boundary_to_structured_ray",
        description="Extend A454 only along the frozen H:O:B=m:m:1 topology-and-phase family.",
        pattern=["A454_frequency_boundary"],
        conclusion="A456_selected_no_refit_frequency_ray_pattern",
    ...
    writer.add_rule(
        ...
        pattern=["A456_selected_no_refit_frequency_ray_pattern"],
        conclusion="A456_exact_W52_frequency_ray_geometry",
        confidence_modifier=1.0,
    )
    writer.add_rule(
        name="exact_geometry_to_decision",
        description="Apply the frozen A454 comparison rule to choose a separate recovery queue or retain BOOHH.",
        pattern=["A456_exact_W52_frequency_ray_geometry"],
        conclusion=f"A456_{decision}",
        confidence_modifier=1.0,
    )
    writer.add_triplet(
        trigger="A454:H2_O2_B1_frequency_boundary",
        mechanism="structured_m3_to_m6_topology_phase_ray_with_fixed_no_refit_measurement",
        outcome="A456:selected_no_refit_frequency_ray_pattern",
        confidence=1.0,
        source=payload["calibration"]["calibration_sha256"],
        quantification=json.dumps(
            payload["calibration"]["per_m_frontier"], sort_keys=True
        ),
        evidence=json.dumps(
    ...
    writer.add_triplet(
        trigger="A456:selected_no_refit_frequency_ray_pattern",
        mechanism="period31_native_first_encounter_compiler",
        outcome="A456:complete_W52_pair_permutation",
        confidence=1.0,
    ...
    writer.add_triplet(
        trigger="A456:complete_W52_pair_permutation",
        mechanism="exhaustive_component_proposal_bound_and_geometry_gate",
        outcome="A456:exact_W52_frequency_ray_geometry",
        confidence=1.0,
    ...
    writer.add_triplet(
        trigger="A456:exact_W52_frequency_ray_geometry",
        mechanism="frozen_A454_minimum_then_aggregate_comparison",
        outcome=f"A456:{decision}",
        confidence=1.0,
    ...
    writer.add_triplet(
        trigger="A454:H2_O2_B1_frequency_boundary",
        mechanism="materialized_frequency_ray_calibration_and_exact_geometry_closure",
        outcome="A456:exact_W52_frequency_ray_geometry",
        confidence=1.0,
        source="materialized:A456_frequency_ray_geometry_chain",
    ...
    writer.add_triplet(
        ...
        outcome=f"A456:{decision}",
        confidence=1.0,
        source="materialized:A456_frequency_ray_decision_chain",
        quantification="exact retained closure",
        evidence=payload["evidence_stage"],
        domain="AI-native retained inference",
        quality_score=1.0,
    ...
    writer.add_cluster(
        name="A456 structured no-refit frequency ray",
        entities=[
            "A454:H2_O2_B1_frequency_boundary",
    ...
    next_type = (
        "exclusive_queued_A456_W52_recovery_execution"
        if decision == "separate_recovery_queue_qualified"
        else "state_adaptive_component_allocation"
    )
    writer.add_gap(
        subject=f"A456:{decision}",
        predicate="next_required_object",
        expected_object_type=next_type,
        confidence=1.0,
        suggested_queries=[
            "Execute the frozen A456 stream after the existing queue."
            if decision == "separate_recovery_queue_qualified"
            else "Use calibration-only prefix state to allocate H, B, and O proposals adaptively while retaining complete-cover guarantees."
        ],
    )
    ...
    if (
        ...
        raise RuntimeError("A456 authentic Causal reopen gate failed")
    return {
...
def _build_result_once(*, expected_implementation_sha256: str) -> dict[str, Any]:
    ...
    _a454, _a448, a449, _a451, _a453 = load_sources()
    ...
    decision = {
        "outcome": (
            ...
            if qualifies
            else "A454_BOOHH_retained"
        ),
        "qualifies_separate_recovery_queue": qualifies,
        "rule": "A456 minimum > A454 minimum, or equal minimum with A456 aggregate > A454 aggregate",
        "selected_delta_over_A454_BOOHH": calibration[
            "selected_delta_over_A454_BOOHH"
        ],
        "W52_target_labels_used": 0,
        "candidate_assignments_executed": 0,
    ...
    if not ready:
        raise RuntimeError("A456 exact materialization gate failed")
    evidence_stage = (
        "STRICT_NO_REFIT_FREQUENCY_RAY_W52_RECOVERY_STREAM_QUALIFIED"
        if qualifies
        else "STRICT_NO_REFIT_FREQUENCY_RAY_BOUNDARY_LOCALIZED_A454_RETAINED"
    )
    core: dict[str, Any] = {
        "schema": "chacha20-round20-w52-no-refit-frequency-ray-portfolio-a456-v1",
        "attempt_id": ATTEMPT_ID,
        ...
        "A456_candidate_results_seen_before_design": False,
        "A456_selected_pattern_seen_before_design": False,
        "A456_W52_pair_stream_seen_before_design": False,
        "A426_A438_A440_A443_A450_A452_A455_secret_result_or_worker_progress_read": False,
        "W52_target_labels_used": 0,
        "feature_refits": 0,
        "model_refits": 0,
    ...
    delta = calibration["selected_delta_over_A454_BOOHH"]
    comparison = geometry["comparison_to_A454_BOOHH_stream"]
    atomic_bytes(
        REPORT,
        (
            "# A456 — structured no-refit frequency-ray portfolio\n\n"
            ...
            f"- Delta over A454 BOOHH aggregate / minimum: **{delta['remaining96_aggregate_bit_gain']:+.12f} / {delta['remaining96_minimum_fixed_block_bit_gain']:+.12f} bits**\n"
            ...
            "- Exact component proposal bounds: **all 3 satisfied over all 16,777,216 cells**\n"
            f"- Spearman / top-65,536 overlap with A454: **{comparison['spearman_rank_correlation']:.12f} / {comparison['top_k_overlap']['65536']['overlap_fraction']:.12f}**\n"
...
def load_result(expected_sha256: str) -> dict[str, Any]:
    ...
    if (
        ...
        is not True
        or value.get("A456_candidate_results_seen_before_design") is not False
        or value.get("A456_selected_pattern_seen_before_design") is not False
        or value.get("A456_W52_pair_stream_seen_before_design") is not False
        or value.get(
        ...
        raise RuntimeError("A456 result semantics differ")
    for row in value["anchors"].values():
        anchor(path_from_ref(row["path"]), row["sha256"])
    _ranks, artifact = artifact_rank_vector(ARTIFACT)
    if artifact["sha256"] != value["stream"]["artifact"]["sha256"]:
        raise RuntimeError("A456 pair-stream hash differs")
    return value
