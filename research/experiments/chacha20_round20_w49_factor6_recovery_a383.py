#!/usr/bin/env python3
"""A383: execute the frozen A382 six-view W49 order with the qualified 64-slab engine."""
...
DESIGN = CONFIGS / "chacha20_round20_w49_factor6_recovery_a383_design_v1.json"
IMPLEMENTATION = CONFIGS / "chacha20_round20_w49_factor6_recovery_a383_implementation_v1.json"
PROTOCOL = CONFIGS / "chacha20_round20_w49_factor6_recovery_a383_v1.json"
PROGRESS = RESULTS / "chacha20_round20_w49_factor6_recovery_a383_progress_v1.json"
RESULT = RESULTS / "chacha20_round20_w49_factor6_recovery_a383_v1.json"
CAUSAL = RESULT.with_suffix(".causal")
REPORT = RESULT.with_suffix(".md")
TEST = ROOT / "tests/test_chacha20_round20_w49_factor6_recovery_a383.py"
REPRO = ROOT / "scripts/reproduce_chacha20_round20_w49_factor6_recovery_a383.sh"
...
A382_DESIGN = (
    CONFIGS / "chacha20_round20_w49_crosswidth_wide_consensus_factor6_a382_design_v1.json"
)
A382_IMPLEMENTATION = (
    CONFIGS
    / "chacha20_round20_w49_crosswidth_wide_consensus_factor6_a382_implementation_v1.json"
)
A382_ORDER = (
...
ATTEMPT_ID = "A383"
DESIGN_SHA256 = "a4b5446f54609c42bcade5b525d813a2b24ea2cbc7430c5f0d170d5d1ece2c9b"
...
A382_DESIGN_SHA256 = "c332e7c4fc8953384a208657a440324d7fa0f98ed6c85efb8d91b6705b8bd3d1"
A382_IMPLEMENTATION_SHA256 = "12be1bf8aec21cd5866708e82792d90f7b6ae111c2e962e9d58ecb3a6c87924c"
...
SELECTED_OPERATOR = "A382_W49_crosswidth_wide_consensus_six_view_factor6_wavefront"
...
def load_module(path: Path, name: str) -> Any:
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot import A383 dependency {path}")
...
A379 = load_module(A379_RUNNER, "a383_a379")
A382 = load_module(A382_RUNNER, "a383_a382")
...
def exact_order(values: Sequence[int], label: str) -> list[int]:
    ...
    if len(order) != CELLS or set(order) != set(range(CELLS)):
        raise ValueError(f"A383 {label} is not an exact 4,096-cell order")
    return order


def order_sha256(order: Sequence[int]) -> str:
    return A382.order_sha256(exact_order(order, "hash"))
...
def load_design() -> dict[str, Any]:
    if file_sha256(DESIGN) != DESIGN_SHA256:
        raise RuntimeError("A383 design hash differs")
    value = json.loads(DESIGN.read_bytes())
    boundary = value.get("information_boundary", {})
    execution = value.get("execution_contract", {})
    order = value.get("order_contract", {})
    if (
        value.get("schema") != "chacha20-round20-w49-factor6-recovery-a383-design-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("design_state")
        != "frozen_after_A378_qualification_A379_protocol_A382_order_before_any_A383_candidate_prefix_or_result"
        ...
        or boundary.get("A382_order_available_at_design_freeze") is not True
        or boundary.get("A383_candidate_or_prefix_available_at_design_freeze") is not False
        ...
        or order.get("source_views") != 6
    ):
        raise RuntimeError("A383 frozen design semantics differ")
...
def assert_pre_execution() -> None:
    if PROGRESS.exists() or RESULT.exists():
        raise RuntimeError("A383 frozen artifacts must precede every W49 candidate and result")


def freeze_implementation() -> dict[str, Any]:
    if any(path.exists() for path in (IMPLEMENTATION, PROTOCOL, PROGRESS, RESULT, CAUSAL, REPORT)):
        raise FileExistsError("A383 implementation or execution artifacts already exist")
    assert_pre_execution()
    load_design()
    if not TEST.exists() or not REPRO.exists():
        raise FileNotFoundError("A383 test and reproducer must precede implementation freeze")
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w49-factor6-recovery-a383-implementation-v1",
        "attempt_id": ATTEMPT_ID,
        "commitment_state": "frozen_after_A378_qualification_A379_protocol_A382_order_before_A383_candidate_prefix_or_result",
        ...
        "A382_order_available_at_implementation_freeze": True,
        "A383_candidate_or_prefix_available_at_implementation_freeze": False,
        "anchors": {
            ...
            "A382_design": anchor(A382_DESIGN, A382_DESIGN_SHA256),
            "A382_implementation": anchor(A382_IMPLEMENTATION, A382_IMPLEMENTATION_SHA256),
            "A382_order": anchor(A382_ORDER, A382_ORDER_SHA256),
            "A382_causal": anchor(A382_CAUSAL, A382_CAUSAL_SHA256),
            "A382_runner": anchor(A382_RUNNER),
...
def load_implementation(expected_sha256: str) -> dict[str, Any]:
    if file_sha256(IMPLEMENTATION) != expected_sha256:
        raise RuntimeError("A383 implementation hash differs")
    value = json.loads(IMPLEMENTATION.read_bytes())
    if (
        value.get("schema")
        != "chacha20-round20-w49-factor6-recovery-a383-implementation-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("commitment_state")
        != "frozen_after_A378_qualification_A379_protocol_A382_order_before_A383_candidate_prefix_or_result"
        ...
        or value.get("A382_order_available_at_implementation_freeze") is not True
    ):
        raise RuntimeError("A383 implementation semantics differ")
    expected = {
        "design": DESIGN,
        ...
        "A382_design": A382_DESIGN,
        "A382_implementation": A382_IMPLEMENTATION,
    ...
    for name, path in expected.items():
        ...
        if row.get("path") != relative(path) or row.get("sha256") != file_sha256(path):
            raise RuntimeError(f"A383 implementation anchor differs: {name}")
    unsigned = {key: item for key, item in value.items() if key != "implementation_commitment_sha256"}
    if value.get("implementation_commitment_sha256") != canonical_sha256(unsigned):
        raise RuntimeError("A383 implementation commitment differs")
    return value


def load_a382_order(expected_sha256: str, expected_a379_protocol_sha256: str) -> dict[str, Any]:
    value = A382.load_order(expected_sha256)
    selected = exact_order(value.get("selected_order", []), "A382 selected order")
    source_orders = {
        role: exact_order(order, f"A382 source {role}")
        for role, order in value.get("source_orders", {}).items()
    }
    if (
        value.get("schema")
        != "chacha20-round20-w49-crosswidth-wide-consensus-factor6-a382-order-v1"
        or value.get("anchors", {}).get("A379_protocol", {}).get("sha256")
        != expected_a379_protocol_sha256
        ...
        or tuple(value.get("source_role_order", [])) != A382.SOURCE_ROLES
        or selected != A382.factor_k_wavefront(source_orders, A382.SOURCE_ROLES)
        or value.get("pointwise_factor6_proof")
        != A382.factor_k_proof(source_orders, A382.SOURCE_ROLES, selected)
        or value.get("information_boundary", {}).get(
            "A381_result_or_true_prefix_used"
        )
        is not False
    ):
        raise RuntimeError("A383 A382 order semantics differ")
    return value
...
def freeze_protocol(
    *,
    ...
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w49-factor6-recovery-a383-protocol-v1",
        "attempt_id": ATTEMPT_ID,
        "protocol_state": "A382_six_view_order_bound_before_any_A383_candidate_prefix_or_result",
        "design_sha256": DESIGN_SHA256,
        ...
        "A382_order_sha256": expected_a382_order_sha256,
        "A382_order_commitment_sha256": order["order_commitment_sha256"],
        ...
        "source_role_order": order["source_role_order"],
        "source_orders_sha256": order["source_orders_sha256"],
        "pointwise_factor6_proof": order["pointwise_factor6_proof"],
        "pairwise_diversity_summary": order["pairwise_diversity_summary"],
        "execution_contract": load_design()["execution_contract"],
        "information_boundary": {
            "A383_candidate_or_prefix_available_at_protocol_freeze": False,
            "A382_order_frozen_before_protocol": True,
        ...
        "anchors": {
            ...
            "A379_order": anchor(A379_ORDER, A379_ORDER_SHA256),
            "A382_order": anchor(A382_ORDER, expected_a382_order_sha256),
            "A382_causal": anchor(A382_CAUSAL, A382_CAUSAL_SHA256),
            "runner": anchor(Path(__file__)),
            "test": anchor(TEST),
            "reproducer": anchor(REPRO),
    ...
    payload["protocol_commitment_sha256"] = canonical_sha256(
        {
            ...
            "A379_protocol_sha256": expected_a379_protocol_sha256,
            "A382_order_sha256": expected_a382_order_sha256,
            "A382_order_commitment_sha256": payload["A382_order_commitment_sha256"],
    ...
    if PROGRESS.exists() or RESULT.exists():
        ...
        raise RuntimeError("A383 protocol did not precede A383 execution")
    return payload


def load_protocol(expected_sha256: str) -> dict[str, Any]:
    if file_sha256(PROTOCOL) != expected_sha256:
        raise RuntimeError("A383 protocol hash differs")
    value = json.loads(PROTOCOL.read_bytes())
    selected = exact_order(value.get("selected_order", []), "stored protocol order")
    if (
        value.get("schema") != "chacha20-round20-w49-factor6-recovery-a383-protocol-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("selected_operator") != SELECTED_OPERATOR
        or value.get("selected_order_uint16be_sha256") != order_sha256(selected)
        or value.get("information_boundary", {}).get(
            "A383_candidate_or_prefix_available_at_protocol_freeze"
        )
        is not False
        ...
        raise RuntimeError("A383 protocol semantics differ")
    for row in value["anchors"].values():
        anchor(path_from_ref(row["path"]), row["sha256"])
    load_implementation(value["implementation_sha256"])
    load_a382_order(value["A382_order_sha256"], value["A379_protocol_sha256"])
...
def load_resume(
    *, protocol_sha256: str, order_sha: str, qualification_sha256: str
    ...
    if (
        value.get("schema") != "chacha20-round20-w49-factor6-recovery-a383-progress-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        ...
        raise RuntimeError("A383 progress fingerprint differs")
    ...
    if not 0 <= completed < CELLS or value.get("factual_filter_candidates") != 0:
        raise RuntimeError("A383 resumable progress state differs")
    return completed, float(value.get("gpu_seconds", 0.0)), int(value.get("host_instances", 0)), None


def rank_panel(prefix: int, protocol: Mapping[str, Any], a382_order: Mapping[str, Any]) -> dict[str, Any]:
    source_orders = {
        role: exact_order(order, f"rank source {role}")
        for role, order in a382_order["source_orders"].items()
    }
    selected = exact_order(protocol["selected_order"], "selected rank source")
    source_ranks = {
        role: rank_vector(source_orders[role])[prefix]
        for role in protocol["source_role_order"]
    }
    factor2_order = exact_order(
        A382.load_a380_order()["selected_order"], "A380/A381 factor-two comparison"
    )
    selected_rank = rank_vector(selected)[prefix]
    factor2_rank = rank_vector(factor2_order)[prefix]
    ranks = {
        **source_ranks,
        "A380_A381_factor2_portfolio": factor2_rank,
        "A383_executed_factor6_portfolio": selected_rank,
    }
    best = min(source_ranks.values())
    return {
        "prefix12": prefix,
        ...
        "factor6_bound_for_confirmed_prefix": selected_rank <= 6 * best,
        "portfolio_overhead_vs_best_source": selected_rank / best,
        "rank_ratio_vs_A380_A381_factor2": factor2_rank / selected_rank,
        "rank_gain_bits_vs_A380_A381_factor2": math.log2(factor2_rank / selected_rank),
...
def build_causal(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    terminal = "A383:confirmed_factor6_W49_recovery"
    writer = CausalWriter(api_id="a383w49")
    writer._rules = []
    writer.add_rule(
        name="crosswidth_readers_to_six_view_bounded_order",
        description=(
            "Four unchanged W46 Readers transfer to the complete W49 field and join "
            "the A379 and A380 orders under an exact pointwise factor-six bound."
        ),
        pattern=["A375_frozen_W46_reader_portfolio", "A380_complete_W49_measurement"],
        conclusion="A383_frozen_W49_recovery_order",
    ...
    writer.add_rule(
        ...
        pattern=["A383_frozen_W49_recovery_order", "A378_exact_W49_group_engine"],
        conclusion="A383_confirmed_factor6_W49_recovery",
        confidence_modifier=1.0,
    )
    writer.add_triplet(
        trigger="A382:crosswidth_six_view_factor6_order",
        mechanism="qualified_ordered_complete_sixtyfour_slab_search",
        outcome="A383:sole_factual_W49_model",
        confidence=1.0,
        source=payload["execution_sha256"],
        quantification=json.dumps(payload["discovery"], sort_keys=True),
        evidence=json.dumps(payload["rank_analysis"], sort_keys=True),
        domain="fresh full-round ChaCha20 W49 cross-width multiview recovery",
        quality_score=1.0,
    )
    writer.add_triplet(
        trigger="A383:sole_factual_W49_model",
    ...
    writer.add_triplet(
        ...
        outcome=terminal,
        confidence=1.0,
        source="materialized:A383_W49_recovery_chain",
    ...
    writer.add_cluster(
        name="A383 factor-six W49 recovery",
        entities=[
            "A382:crosswidth_six_view_factor6_order",
            "A383:sole_factual_W49_model",
            terminal,
        ],
    )
    writer.add_gap(
        ...
        expected_object_type="W50_crosswidth_transfer_or_second_fresh_W49_replication",
        confidence=1.0,
        suggested_queries=[
            "Transfer the frozen multiview portfolio to W50 after exact engine "
            "qualification, or replicate it on a second fresh W49 challenge."
        ],
    )
    ...
    if (
        ...
        raise RuntimeError("A383 authentic Causal reopen gate failed")
    return {
...
def ordered_discovery(
    *,
    ...
    if not 0 <= start_group < CELLS:
        raise ValueError("A383 resume group lies outside the prefix cover")
    ...
    try:
        for group_index in range(start_group, CELLS):
            ...
            if controls:
                raise RuntimeError("A383 matched control produced a candidate")
            if not factual:
                ...
                continue
            if len(factual) != 1:
                raise RuntimeError("A383 complete W49 group produced multiple filters")
            candidate = factual[0]
            if ((candidate >> WORD0_SUFFIX_BITS) & (CELLS - 1)) != prefix:
                raise RuntimeError("A383 candidate prefix differs")
            found = {
                "candidate": candidate,
        ...
        if host is not None:
            host.close()
    raise RuntimeError("A383 exact frozen order exhausted without a factual filter")


def recover(*, expected_protocol_sha256: str) -> dict[str, Any]:
    if any(path.exists() for path in (RESULT, CAUSAL, REPORT)):
        raise FileExistsError("A383 result artifacts already exist")
    ...
    a382_order = load_a382_order(protocol["A382_order_sha256"], protocol["A379_protocol_sha256"])
    ...
    def write_progress(row: Mapping[str, Any]) -> None:
        atomic_json(
            PROGRESS,
            {
                "schema": "chacha20-round20-w49-factor6-recovery-a383-progress-v1",
                "attempt_id": ATTEMPT_ID,
    ...
    if confirmation["all_blocks_match"] is not True:
        raise RuntimeError("A383 dual independent confirmation failed")
    ranks = rank_panel(int(discovery["prefix12"]), protocol, a382_order)
    if ranks["selected_rank_one_based"] != discovery["executed_prefix_groups"]:
        raise RuntimeError("A383 discovery rank differs from selected order")
    strict_subset = discovery["executed_prefix_groups"] < CELLS
    evidence_stage = (
        "FULLROUND_R20_FACTOR6_W49_STRICT_SUBSET_RECOVERY_CONFIRMED"
        if strict_subset
        else "FULLROUND_R20_FACTOR6_W49_COMPLETE_DOMAIN_RECOVERY_CONFIRMED"
    )
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w49-factor6-recovery-a383-result-v1",
        "attempt_id": ATTEMPT_ID,
        ...
        "A382_order_sha256": protocol["A382_order_sha256"],
        "A382_order_commitment_sha256": protocol["A382_order_commitment_sha256"],
        "public_challenge_sha256": protocol["public_challenge_sha256"],
        "selected_operator": SELECTED_OPERATOR,
        "selected_order_uint16be_sha256": protocol["selected_order_uint16be_sha256"],
        "source_role_order": protocol["source_role_order"],
        "source_orders_sha256": protocol["source_orders_sha256"],
        "pointwise_factor6_proof": protocol["pointwise_factor6_proof"],
        "pairwise_diversity_summary": protocol["pairwise_diversity_summary"],
        "qualification_gate": {
            "evidence_stage": qualification["evidence_stage"],
            "qualification_sha256": qualification["qualification_sha256"],
            "complete_W49_group_candidates": qualification["complete_group_gate"][
                "logical_candidates"
            ],
    ...
    payload["measurement_sha256"] = canonical_sha256(
        {
            ...
            "pointwise_factor6_proof": payload["pointwise_factor6_proof"],
            "information_boundary": payload["information_boundary"],
        }
    )
    ...
    atomic_bytes(
        REPORT,
        (
            "# A383 — six-view full-round ChaCha20 W49 recovery\n\n"
            ...
            f"- A380/A381 factor-two comparison rank: "
            f"**{ranks['prefix_ranks_one_based']['A380_A381_factor2_portfolio']} / {CELLS}**\n"
...
def main() -> None:
    ...
    parser.add_argument("--expected-a382-order-sha256")
    ...
    if args.freeze_implementation:
        ...
        required = (
            ...
            args.expected_a382_order_sha256,
            args.expected_a378_qualification_sha256,
        )
        if not all(required):
            parser.error(
                "--freeze-protocol requires implementation, A379, A382 and qualification hashes"
            )
        payload = freeze_protocol(
            expected_implementation_sha256=args.expected_implementation_sha256,
            expected_a379_protocol_sha256=args.expected_a379_protocol_sha256,
            expected_a382_order_sha256=args.expected_a382_order_sha256,
