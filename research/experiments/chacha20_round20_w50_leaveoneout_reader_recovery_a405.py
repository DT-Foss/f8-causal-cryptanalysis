#!/usr/bin/env python3
"""A405: execute an out-of-fold-qualified A404 W50 production order."""
...
DESIGN = CONFIGS / "chacha20_round20_w50_leaveoneout_reader_recovery_a405_design_v1.json"
IMPLEMENTATION = (
    CONFIGS / "chacha20_round20_w50_leaveoneout_reader_recovery_a405_implementation_v1.json"
)
PROTOCOL = CONFIGS / "chacha20_round20_w50_leaveoneout_reader_recovery_a405_v1.json"
PROGRESS = RESULTS / "chacha20_round20_w50_leaveoneout_reader_recovery_a405_progress_v1.json"
RESULT = RESULTS / "chacha20_round20_w50_leaveoneout_reader_recovery_a405_v1.json"
CAUSAL = RESULT.with_suffix(".causal")
REPORT = RESULT.with_suffix(".md")
TEST = ROOT / "tests/test_chacha20_round20_w50_leaveoneout_reader_recovery_a405.py"
REPRO = ROOT / "scripts/reproduce_chacha20_round20_w50_leaveoneout_reader_recovery_a405.sh"
...
A404_DESIGN = CONFIGS / "chacha20_round20_w50_knownkey_leaveoneout_reader_a404_design_v1.json"
A404_IMPLEMENTATION = (
...
ATTEMPT_ID = "A405"
DESIGN_SHA256 = "b39ae3b7dcc592ca17e39c7a3b8478a1f6decf994bd91c97aeb1d86af3e038dc"
...
A404_DESIGN_SHA256 = "ecd0eb3d7f7b155dc4e87d56335890889eaa3dde57c6d3749e72cf321e71d1a1"
...
SELECTED_OPERATOR = "A404_sixteen_fold_outoffold_qualified_fullfit_production_reader"
CELLS = 4096
GROUP_SIZE = 1 << 38
DOMAIN_SIZE = 1 << 50
...
def load_module(path: Path, name: str) -> Any:
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot import A405 dependency {path}")
...
A391 = load_module(A391_RUNNER, "a405_a391")
A404 = load_module(A404_RUNNER, "a405_a404")
...
def exact_order(values: Sequence[int], label: str) -> list[int]:
    return A391.exact_order(values, f"A405 {label}")
...
def execution_decision(
    learned_order: Sequence[int], comparison_order: Sequence[int] | None
) -> dict[str, Any]:
    learned_sha = order_sha256(learned_order)
    comparison_sha = order_sha256(comparison_order) if comparison_order is not None else None
    duplicate = comparison_sha == learned_sha
    return {
        "execution_enabled": not duplicate,
        "duplicate_A402_order": duplicate,
        "selected_order_uint16be_sha256": learned_sha,
        "A402_comparison_order_uint16be_sha256": comparison_sha,
    }


def load_design() -> dict[str, Any]:
    if file_sha256(DESIGN) != DESIGN_SHA256:
        raise RuntimeError("A405 design hash differs")
    ...
    if (
        value.get("schema") != "chacha20-round20-w50-leaveoneout-reader-recovery-a405-design-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("design_state")
        != "frozen_during_A401_measurement_after_A404_implementation_freeze_before_A401_A402_A404_results_or_any_A405_candidate_prefix"
        or source.get("required_A404_qualification") is not True
        or source.get("required_order_cells") != CELLS
        or source.get("target_labels_used_for_order") != 0
        or duplicate.get("A402_result_must_exist_before_A405_protocol_decision") is not True
        ...
        or boundary.get("A404_result_or_production_order_available_at_design_freeze") is not False
        or boundary.get("A405_candidate_or_prefix_available_at_design_freeze") is not False
    ):
        raise RuntimeError("A405 frozen design semantics differ")
...
def freeze_implementation() -> dict[str, Any]:
    ...
    if any(path.exists() for path in outputs):
        raise FileExistsError("A405 implementation or execution artifact already exists")
    if (
        A404_RESULT.exists()
        or A402_RESULT.exists()
        or A404.A401.RESULT.exists()
        or A404.A401.SELECTION.exists()
    ):
        raise RuntimeError("A405 code freeze must precede A401, A402 and A404 results")
    load_design()
    A391.A389.load_sources()
    if not TEST.exists() or not REPRO.exists():
        raise FileNotFoundError("A405 test and reproducer must precede freeze")
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w50-leaveoneout-reader-recovery-a405-implementation-v1",
        "attempt_id": ATTEMPT_ID,
        "commitment_state": "complete_executor_and_dedup_gate_frozen_before_A401_A402_A404_results_or_A405_candidate",
        "design_sha256": DESIGN_SHA256,
        "selected_operator": SELECTED_OPERATOR,
        "A401_A402_A404_results_available_at_freeze": False,
        "A405_candidate_or_prefix_available_at_freeze": False,
        "anchors": {
            "design": anchor(DESIGN, DESIGN_SHA256),
            "A391_runner": anchor(A391_RUNNER, A391_RUNNER_SHA256),
            "A404_design": anchor(A404_DESIGN, A404_DESIGN_SHA256),
            "A404_runner": anchor(A404_RUNNER, A404_RUNNER_SHA256),
            "A404_implementation": anchor(A404_IMPLEMENTATION, A404_IMPLEMENTATION_SHA256),
...
def load_implementation(expected_sha256: str) -> dict[str, Any]:
    if file_sha256(IMPLEMENTATION) != expected_sha256:
        raise RuntimeError("A405 implementation file hash differs")
    value = json.loads(IMPLEMENTATION.read_bytes())
    if (
        value.get("schema")
        != "chacha20-round20-w50-leaveoneout-reader-recovery-a405-implementation-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("commitment_state")
        != "complete_executor_and_dedup_gate_frozen_before_A401_A402_A404_results_or_A405_candidate"
        or value.get("selected_operator") != SELECTED_OPERATOR
        or value.get("A401_A402_A404_results_available_at_freeze") is not False
        or value.get("A405_candidate_or_prefix_available_at_freeze") is not False
    ):
        raise RuntimeError("A405 implementation semantics differ")
    ...
    if value.get("implementation_commitment_sha256") != canonical_sha256(unsigned):
        raise RuntimeError("A405 implementation commitment differs")
    return value


def load_a404_order(expected_result_sha256: str) -> tuple[dict[str, Any], list[int]]:
    if file_sha256(A404_RESULT) != expected_result_sha256:
        raise RuntimeError("A405 A404 result hash differs")
    value = json.loads(A404_RESULT.read_bytes())
    order = exact_order(value.get("production_order", []), "A404 production order")
    ...
    if (
        ...
        or value.get("evidence_stage") != source["required_A404_evidence_stage"]
        ...
        raise RuntimeError("A405 A404 qualified-order semantics differ")
    return value, order


def load_a402_comparison(expected_result_sha256: str) -> tuple[dict[str, Any], list[int] | None]:
    if file_sha256(A402_RESULT) != expected_result_sha256:
        raise RuntimeError("A405 A402 comparison result hash differs")
    value = json.loads(A402_RESULT.read_bytes())
    qualified = value.get("qualification", {}).get("qualified") is True
    order = (
        exact_order(value.get("production_order", []), "A402 comparison order")
        if qualified
        else None
    )
    expected_stage = (
    ...
    if (
        ...
        or value.get("evidence_stage") != expected_stage
        or (not qualified and value.get("production_order") is not None)
        or value.get("production_order_uint16be_sha256")
        != (order_sha256(order) if order is not None else None)
        or value.get("production_target_labels_used") != 0
        or value.get("production_reader_refits") != 0
        or value.get("production_candidate_assignments_executed") != 0
        or value.get("new_solver_stages") != 0
        ...
        raise RuntimeError("A405 A402 comparison semantics differ")
    return value, order
...
def freeze_protocol(
    *,
    ...
    if any(path.exists() for path in (PROTOCOL, PROGRESS, RESULT, CAUSAL, REPORT)):
        raise FileExistsError("A405 protocol or execution artifact already exists")
    implementation = load_implementation(expected_implementation_sha256)
    a404, learned_order = load_a404_order(expected_a404_result_sha256)
    a402, a402_order = load_a402_comparison(expected_a402_result_sha256)
    decision = execution_decision(learned_order, a402_order)
    duplicate = decision["duplicate_A402_order"]
    execution_enabled = decision["execution_enabled"]
    source, _a388, qualification, _source_orders = A391.A389.load_sources()
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w50-leaveoneout-reader-recovery-a405-protocol-v1",
        "attempt_id": ATTEMPT_ID,
        "protocol_state": (
            "outoffold_qualified_distinct_A404_order_bound_before_any_A405_candidate"
            if execution_enabled
            else "outoffold_qualified_A404_order_byte_identical_to_A402_equivalence_only"
        ),
        ...
        "A404_deployment_commitment_sha256": a404["deployment_commitment_sha256"],
        "A402_result_sha256": expected_a402_result_sha256,
        "A402_qualified": a402["qualification"]["qualified"],
        "A402_comparison_order_uint16be_sha256": decision["A402_comparison_order_uint16be_sha256"],
        ...
        "selected_operator": SELECTED_OPERATOR,
        "selected_order_uint16be_sha256": decision["selected_order_uint16be_sha256"],
        "selected_order": learned_order,
        "execution_contract": load_design()["execution_contract"],
        "information_boundary": {
            "A405_candidate_or_prefix_available_at_protocol_freeze": False,
            "A404_order_bound_without_mutation": True,
    ...
    payload["protocol_commitment_sha256"] = canonical_sha256(
        {
            ...
            "A404_deployment_commitment_sha256": payload["A404_deployment_commitment_sha256"],
            "A402_result_sha256": payload["A402_result_sha256"],
            "duplicate_A402_order": duplicate,
            "execution_enabled": execution_enabled,
...
def load_protocol(expected_sha256: str) -> dict[str, Any]:
    if file_sha256(PROTOCOL) != expected_sha256:
        raise RuntimeError("A405 protocol file hash differs")
    value = json.loads(PROTOCOL.read_bytes())
    order = exact_order(value.get("selected_order", []), "protocol order")
    duplicate = value.get("duplicate_A402_order") is True
    expected_state = (
        "outoffold_qualified_A404_order_byte_identical_to_A402_equivalence_only"
        if duplicate
        else "outoffold_qualified_distinct_A404_order_bound_before_any_A405_candidate"
    )
    expected_commitment = canonical_sha256(
        {
            "implementation_commitment_sha256": value.get("implementation_commitment_sha256"),
            "A404_deployment_commitment_sha256": value.get("A404_deployment_commitment_sha256"),
            "A402_result_sha256": value.get("A402_result_sha256"),
            "duplicate_A402_order": duplicate,
    ...
    if (
        value.get("schema") != "chacha20-round20-w50-leaveoneout-reader-recovery-a405-protocol-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("selected_operator") != SELECTED_OPERATOR
        or value.get("protocol_state") != expected_state
        or value.get("execution_enabled") is duplicate
        ...
        or value.get("information_boundary", {}).get(
            "A405_candidate_or_prefix_available_at_protocol_freeze"
        )
        is not False
        ...
        raise RuntimeError("A405 protocol semantics differ")
    for row in value["anchors"].values():
        anchor(path_from_ref(row["path"]), row["sha256"])
    load_implementation(value["implementation_sha256"])
    a404, expected_order = load_a404_order(value["A404_result_sha256"])
    a402, comparison_order = load_a402_comparison(value["A402_result_sha256"])
    decision = execution_decision(expected_order, comparison_order)
    expected_duplicate = decision["duplicate_A402_order"]
    if (
        order != expected_order
        or duplicate != expected_duplicate
        or value["execution_enabled"] != decision["execution_enabled"]
        or value["A402_qualified"] != a402["qualification"]["qualified"]
        or value["A402_comparison_order_uint16be_sha256"]
        != decision["A402_comparison_order_uint16be_sha256"]
        or value["A404_deployment_commitment_sha256"] != a404["deployment_commitment_sha256"]
    ):
        raise RuntimeError("A405 protocol no longer equals its sources")
...
def load_resume(
    *, protocol_sha256: str, order_sha: str, qualification_sha256: str
    ...
    if (
        value.get("schema") != "chacha20-round20-w50-leaveoneout-reader-recovery-a405-progress-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        ...
        raise RuntimeError("A405 progress fingerprint differs")
    ...
    if not 0 <= completed < CELLS or value.get("factual_filter_candidates") != 0:
        raise RuntimeError("A405 resumable progress state differs")
    return (
        completed,
...
def rank_panel(prefix: int, learned: Sequence[int]) -> dict[str, Any]:
    ...
    return {
        ...
        "A405_leaveoneout_rank_one_based": learned_rank,
        "A391_Direct12_comparison_rank_one_based": direct_rank,
        "A388_factor3_comparison_rank_one_based": factor3_rank,
        "leaveoneout_gain_bits_vs_complete_domain": math.log2(CELLS / learned_rank),
        "leaveoneout_domain_reduction_factor": CELLS / learned_rank,
        "leaveoneout_rank_ratio_to_Direct12": learned_rank / direct_rank,
        "leaveoneout_rank_ratio_to_A388_factor3": learned_rank / factor3_rank,
...
def build_causal(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    terminal = "A405:confirmed_leaveoneout_reader_W50_recovery"
    writer = CausalWriter(api_id="a405w50")
    writer._rules = []
    writer.add_rule(
        name="outoffold_reader_and_complete_engine_to_model",
        description="The A404 out-of-fold-qualified order executes complete W50 groups until the sole factual model appears.",
        pattern=["A404_outoffold_qualified_production_order", "A384_exact_W50_group_engine"],
        conclusion="A405_sole_factual_W50_model",
    ...
    writer.add_rule(
        ...
        pattern=["A405_sole_factual_W50_model"],
        conclusion="A405_confirmed_leaveoneout_reader_W50_recovery",
        confidence_modifier=1.0,
    )
    writer.add_triplet(
        trigger="A404:outoffold_qualified_production_order",
        mechanism="qualified_complete_onehundredtwentyeight_slab_group_search",
        outcome="A405:sole_factual_W50_model",
        confidence=1.0,
        source=payload["execution_sha256"],
        quantification=json.dumps(payload["discovery"], sort_keys=True),
        evidence=json.dumps(payload["rank_analysis"], sort_keys=True),
        domain="fresh full-round ChaCha20 W50 leave-one-out Reader recovery",
        quality_score=1.0,
    )
    writer.add_triplet(
        trigger="A405:sole_factual_W50_model",
        ...
        domain="confirmed full-round ChaCha20 W50 leave-one-out Reader recovery",
        quality_score=1.0,
    )
    writer.add_triplet(
        trigger="A404:outoffold_qualified_production_order",
        mechanism="materialized_leaveoneout_search_and_confirmation_closure",
        outcome=terminal,
        confidence=1.0,
        source="materialized:A405_leaveoneout_reader_W50_recovery_chain",
    ...
    writer.add_cluster(
        name="A405 out-of-fold-qualified learned W50 recovery",
        entities=[
            "A404:outoffold_qualified_production_order",
            "A405:sole_factual_W50_model",
            terminal,
        ],
    )
    writer.add_gap(
        ...
        expected_object_type="cross_target_outoffold_rank_curve_or_W51_transfer",
        confidence=1.0,
        suggested_queries=[
            "Compare the confirmed production rank to all sixteen out-of-fold ranks and transfer after W51 qualification."
        ],
    )
    ...
    if (
        ...
        raise RuntimeError("A405 authentic Causal reopen gate failed")
    return {
...
def recover(*, expected_protocol_sha256: str) -> dict[str, Any]:
    if any(path.exists() for path in (RESULT, CAUSAL, REPORT)):
        raise FileExistsError("A405 result artifact already exists")
    protocol = load_protocol(expected_protocol_sha256)
    if protocol["execution_enabled"] is not True:
        raise RuntimeError("A405 duplicate order is equivalence-only; execution is disabled")
    ...
    def write_progress(row: Mapping[str, Any]) -> None:
        atomic_json(
            PROGRESS,
            {
                "schema": "chacha20-round20-w50-leaveoneout-reader-recovery-a405-progress-v1",
                "attempt_id": ATTEMPT_ID,
    ...
    if discovery["matched_control_candidates"] != 0:
        raise RuntimeError("A405 matched control produced a candidate")
    ...
    if confirmation["all_blocks_match"] is not True:
        raise RuntimeError("A405 dual independent confirmation failed")
    ranks = rank_panel(int(discovery["prefix12"]), protocol["selected_order"])
    if ranks["A405_leaveoneout_rank_one_based"] != discovery["executed_prefix_groups"]:
        raise RuntimeError("A405 discovery rank differs from leave-one-out order")
    strict_subset = discovery["executed_prefix_groups"] < CELLS
    evidence_stage = (
        "FULLROUND_R20_OUTOFFOLD_QUALIFIED_LEARNED_W50_STRICT_SUBSET_RECOVERY_CONFIRMED"
        if strict_subset
        else "FULLROUND_R20_OUTOFFOLD_QUALIFIED_LEARNED_W50_COMPLETE_DOMAIN_RECOVERY_CONFIRMED"
    )
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w50-leaveoneout-reader-recovery-a405-result-v1",
        "attempt_id": ATTEMPT_ID,
        ...
        "A404_deployment_commitment_sha256": protocol["A404_deployment_commitment_sha256"],
    ...
    atomic_bytes(
        REPORT,
        (
            "# A405 — Out-of-fold-qualified full-round ChaCha20 W50 recovery\n\n"
            f"Evidence stage: **{evidence_stage}**\n\n"
            f"- Learned execution rank: **{ranks['A405_leaveoneout_rank_one_based']} / {CELLS}**\n"
...
def analyze() -> dict[str, Any]:
    payload: dict[str, Any] = {
        ...
        "A404_result_available": A404_RESULT.exists(),
    ...
    if PROTOCOL.exists():
        ...
        payload["duplicate_A402_order"] = value["duplicate_A402_order"]
    if PROGRESS.exists():
...
def main() -> None:
    ...
    if args.freeze_implementation:
        ...
        if not all(required):
            parser.error("--freeze-protocol requires three expected SHA-256 arguments")
