#!/usr/bin/env python3
"""A452: execute A451's non-factorized bounded-regret W52 pair stream."""
...
STEM = "chacha20_round20_w52_deduplicated_reader_portfolio_recovery_a452"
...
A451_DESIGN = (
    CONFIGS
    / "chacha20_round20_w52_deduplicated_reader_portfolio_a451_design_v1.json"
)
A451_IMPLEMENTATION = (
    CONFIGS
    / "chacha20_round20_w52_deduplicated_reader_portfolio_a451_implementation_v1.json"
)
A451_RESULT = (
...
A451_CAUSAL = A451_RESULT.with_suffix(".causal")
A451_READBACK = RESEARCH / "reports/CAUSAL_CHACHA20_A451_PERSONAL_READER_READBACK_V1.md"
A434_QUALIFICATION = (
...
ATTEMPT_ID = "A452"
DESIGN_SHA256 = "be530f5a4f0070a76ff95d045368ae4ac18531fb8535e25c6041358748316940"
...
A451_DESIGN_SHA256 = "600408b0ca67b263503cc2d1bb75593a16b03c50df556af121238e83fcd4b9d9"
A451_IMPLEMENTATION_SHA256 = (
    "7eeb17ca7a9536f925ebebf89f200cb1dcfc9e0d04815f5edf172170ce17e934"
)
A451_RESULT_SHA256 = "f2501e5e85f6d37305473738bb0840c12651720e6f7e3fbab2fc4a253b40bdf6"
A451_CAUSAL_SHA256 = "07d34f4edbdb7d23d97d6b15fb66c21814c1315e3e2ef225f1d57b9e22bf2895"
A451_READBACK_SHA256 = "c85df1015951ab5f2e52fa141ae8d6622f0e7ae302091ff1f7d818898b412b94"
...
A451_FUSION_SHA256 = "83aee1592fa8916545786143888cc233713f429c3bad4e6ba9918b12fa888f56"
A451_GUARANTEE_SHA256 = "660e2a2c9338b685f4bb22a341c0698a689c053d19c77eb9317958be4d4ac220"
...
def load_module(path: Path, name: str) -> Any:
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot import A452 dependency {path}")
...
A451 = load_module(A451_RUNNER, "a452_a451")
file_sha256 = A451.file_sha256
canonical_sha256 = A451.canonical_sha256
atomic_json = A451.atomic_json
atomic_bytes = A451.atomic_bytes
anchor = A451.anchor
path_from_ref = A451.path_from_ref
relative = A451.relative
_A435: Any | None = None
_PAIR_STREAM: PairStream | None = None


class PairStream:
    """Read-only random access over A451's packed pair permutation."""

    def __init__(self, path: Path) -> None:
        if path.stat().st_size != PAIR_CELLS * 4:
            raise RuntimeError("A452 pair stream size differs")
        self.path = path
    ...
    def pair_at(self, global_index: int) -> tuple[int, int]:
        if not 0 <= global_index < PAIR_CELLS:
            raise ValueError("A452 global pair index differs")
        packed = int(self._mapped[global_index])
        prefix = packed >> 16
        off_axis = packed & 0xFFFF
        if prefix >= AXIS_CELLS or off_axis >= AXIS_CELLS:
            raise RuntimeError("A452 packed pair coordinate differs")
    ...
    def global_index(self, prefix: int, off_axis: int) -> int:
        if not 0 <= prefix < AXIS_CELLS or not 0 <= off_axis < AXIS_CELLS:
            raise ValueError("A452 pair coordinate differs")
        ...
        for start in range(0, PAIR_CELLS, 1 << 20):
            ...
            if hits.size:
                if hits.size != 1:
                    raise RuntimeError("A452 pair stream inverse is not unique")
                return start + int(hits[0])
        raise RuntimeError("A452 pair absent from complete stream")
...
def pair_stream() -> PairStream:
    ...
    if _PAIR_STREAM is None:
        ...
        _PAIR_STREAM = PairStream(A451_PAIR_STREAM)
    return _PAIR_STREAM
...
def runtime_a435() -> Any:
    ...
    if _A435 is None:
        ...
        _A435 = load_module(A435_RUNNER, "a452_a435_runtime")
    return _A435


def progress_path(worker_index: int) -> Path:
    if not 0 <= worker_index < WORKERS:
        raise ValueError("A452 worker index differs")
...
def load_design() -> dict[str, Any]:
    ...
    if (
        value.get("schema")
        != "chacha20-round20-w52-deduplicated-reader-portfolio-recovery-a452-design-v1"
        ...
        or schedule.get("source_attempt") != "A451"
        or schedule.get("source_result_sha256") != A451_RESULT_SHA256
        or schedule.get("source_result_commitment_sha256")
        != A451_RESULT_COMMITMENT_SHA256
        or schedule.get("source_pair_stream_sha256")
        != A451_PAIR_STREAM_SHA256
        ...
        or engine.get("A451_nonfactorized_pair_stream_used") is not True
        ...
        or queue.get("A452_runs_after_terminal_A450") is not True
        or queue.get("A452_runs_after_A450_supervisor_and_workers_close") is not True
        or boundary.get(
            "A451_result_and_authentic_Causal_personally_read_at_design_freeze"
        )
        is not True
        ...
        is not False
        or boundary.get("A452_candidate_or_progress_available_at_design_freeze")
        is not False
        or boundary.get("A452_target_labels_used_for_schedule") != 0
        or boundary.get("A452_feature_refits") != 0
        or boundary.get("A452_model_refits") != 0
        or boundary.get("prior_live_recovery_filter_outcomes_consumed") is not False
    ):
        raise RuntimeError("A452 frozen design semantics differ")
...
def load_source_schedule() -> tuple[dict[str, Any], PairStream, tuple[Any, ...]]:
    ...
    anchor(A451_DESIGN, A451_DESIGN_SHA256)
    anchor(A451_IMPLEMENTATION, A451_IMPLEMENTATION_SHA256)
    anchor(A451_RESULT, A451_RESULT_SHA256)
    anchor(A451_PAIR_STREAM, A451_PAIR_STREAM_SHA256)
    anchor(A451_CAUSAL, A451_CAUSAL_SHA256)
    anchor(A451_READBACK, A451_READBACK_SHA256)
    packaged = A451.load_result(A451_RESULT_SHA256)
    if (
        packaged.get("schema")
        ...
        != "TARGET_BLIND_NONFACTORIZED_W52_BOUNDED_REGRET_RECOVERY_STREAM_READY"
        or packaged.get("fusion_algorithm")
        != "deduplicated_fixed_slot_round_robin_min_rank"
        or packaged.get("artifact", {}).get("sha256")
        != A451_PAIR_STREAM_SHA256
        or packaged.get("artifact", {}).get("pair_cells") != PAIR_CELLS
        or packaged.get("artifact", {}).get("complete_permutation") is not True
        or packaged.get("hard_rank_guarantee", {}).get("violations") != 0
        or packaged.get("hard_rank_guarantee", {}).get("guarantee_satisfied")
        is not True
        or packaged.get("W52_target_labels_used") != 0
        or packaged.get("feature_refits") != 0
        or packaged.get("model_refits") != 0
        or packaged.get("production_candidate_assignments_executed") != 0
        or packaged.get("A450_candidate_progress_or_result_read") is not False
        or packaged.get(
            "A426_A438_A440_A443_secret_result_stop_or_worker_progress_read"
        )
        is not False
    ):
        raise RuntimeError("A452 A451 source schedule differs")
    stream = pair_stream()
    if stream.pair_at(0) != tuple(packaged["artifact"]["first_pair"]):
        raise RuntimeError("A452 A451 first pair differs")
    if stream.pair_at(PAIR_CELLS - 1) != tuple(
        packaged["artifact"]["last_pair"]
    ):
        raise RuntimeError("A452 A451 last pair differs")
...
def load_public_challenge() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("A452 A426 public challenge scope differs")
    return a426, challenge


def freeze_implementation() -> dict[str, Any]:
    if IMPLEMENTATION.exists() or not no_downstream_artifacts():
        raise FileExistsError("A452 implementation or downstream artifact exists")
    design = load_design()
    source, stream, _unused = load_source_schedule()
    a426, _challenge = load_public_challenge()
    if not TEST.exists() or not REPRO.exists():
        raise FileNotFoundError("A452 tests and reproducer must precede freeze")
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w52-deduplicated-reader-portfolio-recovery-a452-implementation-v1",
        "attempt_id": ATTEMPT_ID,
        "commitment_state": "complete_restart_safe_eight_worker_A451_deduplicated_reader_portfolio_executor_frozen_without_reading_any_prior_secret_result_stop_or_worker_progress",
        ...
        "source_A451_result_sha256": A451_RESULT_SHA256,
        "source_A451_result_commitment_sha256": source[
            "result_commitment_sha256"
        ],
        "source_A451_pair_stream_sha256": source[
            "artifact"
        ][
            "sha256"
        ],
        "source_A451_fusion_sha256": source["fusion_sha256"],
        "source_A451_guarantee_sha256": source["guarantee_sha256"],
        "source_A451_pair_stream_bytes": source["artifact"]["bytes"],
        ...
        "A452_candidate_or_progress_available_at_freeze": False,
        ...
        "anchors": {
            ...
            "A451_design": anchor(A451_DESIGN, A451_DESIGN_SHA256),
            "A451_implementation": anchor(
                A451_IMPLEMENTATION, A451_IMPLEMENTATION_SHA256
            ),
            "A451_result": anchor(A451_RESULT, A451_RESULT_SHA256),
            "A451_pair_stream": anchor(
                A451_PAIR_STREAM, A451_PAIR_STREAM_SHA256
            ),
            "A451_causal": anchor(A451_CAUSAL, A451_CAUSAL_SHA256),
            "A451_personal_readback": anchor(
                A451_READBACK, A451_READBACK_SHA256
            ),
...
def load_implementation(expected_sha256: str) -> dict[str, Any]:
    ...
    if (
        value.get("schema")
        != "chacha20-round20-w52-deduplicated-reader-portfolio-recovery-a452-implementation-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("design_sha256") != DESIGN_SHA256
        or value.get("source_A451_result_sha256") != A451_RESULT_SHA256
        or value.get("source_A451_result_commitment_sha256")
        != A451_RESULT_COMMITMENT_SHA256
        or value.get("source_A451_pair_stream_sha256")
        != A451_PAIR_STREAM_SHA256
        or value.get("source_A451_pair_stream_bytes") != PAIR_CELLS * 4
        or value.get("source_A451_fusion_sha256") != A451_FUSION_SHA256
        or value.get("source_A451_guarantee_sha256")
        != A451_GUARANTEE_SHA256
        ...
        or value.get("A426_A438_A440_A443_secret_result_stop_or_worker_progress_read")
        is not False
        or value.get("A452_candidate_or_progress_available_at_freeze") is not False
        ...
        raise RuntimeError("A452 implementation semantics differ")
    ...
    if value.get("implementation_commitment_sha256") != canonical_sha256(unsigned):
        raise RuntimeError("A452 implementation commitment differs")
    return value
...
def square_pair_at(
    ...
    if not isinstance(prefix_order, PairStream):
        raise TypeError("A452 pair source is not the frozen PairStream")
...
def pair_global_index(
    ...
    if prefix_order.pair_at(index) != (prefix, off_axis):
        raise RuntimeError("A452 pair-stream inverse identity failed")
    return index
...
def factorized_pair_global_index(
    ...
    if (
        ...
        raise ValueError("A452 factorized comparison order differs")
...
def freeze_protocol(*, expected_implementation_sha256: str) -> dict[str, Any]:
    if not no_downstream_artifacts():
        raise FileExistsError("A452 protocol or execution artifact exists")
    ...
    schedule = {
        "algorithm": "A451_deduplicated_fixed_slot_round_robin_min_rank_pair_stream",
        "selected_reader_portfolio": list(A451.COMPONENTS),
        "pair_stream_sha256": A451_PAIR_STREAM_SHA256,
        "pair_stream_bytes": PAIR_CELLS * 4,
        "A451_result_commitment_sha256": A451_RESULT_COMMITMENT_SHA256,
        "A451_fusion_sha256": A451_FUSION_SHA256,
        "A451_guarantee_sha256": A451_GUARANTEE_SHA256,
        "exact_rank_guarantee": source["hard_rank_guarantee"]["formula"],
        "exact_rank_guarantee_violations": source["hard_rank_guarantee"][
            "violations"
        ],
        "maximum_observed_rank_ratio": source["hard_rank_guarantee"][
            "maximum_observed_rank_ratio"
        ],
        "pair_spearman_to_A442": source["geometry"][
            "comparison_to_A442_borda_sum_baseline"
        ]["spearman_rank_correlation"],
        "top65536_intersection_with_A442": source["geometry"][
            "comparison_to_A442_borda_sum_baseline"
        ...
        "worker_role_formula": "deduplicated_portfolio_wave_{worker_index}",
        "pair_at_formula": "read_uint32be(A451_pair_stream,4*global_index) -> prefix16 || off_axis16",
    ...
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w52-deduplicated-reader-portfolio-recovery-a452-protocol-v1",
        "attempt_id": ATTEMPT_ID,
        "protocol_state": "A451_exact_nonfactorized_bounded_regret_pair_stream_and_A426_public_challenge_bound_before_any_A450_candidate_progress_or_result",
        "design_sha256": DESIGN_SHA256,
        "implementation_sha256": expected_implementation_sha256,
        "implementation_commitment_sha256": implementation[
            "implementation_commitment_sha256"
        ],
        "A451_result_sha256": A451_RESULT_SHA256,
        "A451_result_commitment_sha256": A451_RESULT_COMMITMENT_SHA256,
        ...
        "worker_roles": [f"deduplicated_portfolio_wave_{index}" for index in range(WORKERS)],
        "information_boundary": {
            "A451_schedule_frozen_before_A452_challenge_binding": True,
            "A434_qualification_content_read_at_A452_protocol_freeze": False,
            "A426_assignment_absent_from_protocol": True,
            "A426_A438_A440_A443_secret_result_stop_or_worker_progress_read": False,
            "A450_candidate_progress_or_result_read": False,
            "A452_candidate_or_progress_available_at_protocol_freeze": False,
            "A452_target_labels_used_for_schedule": 0,
            "A452_feature_refits": 0,
            "A452_model_refits": 0,
            "prior_live_recovery_filter_outcomes_consumed": False,
        },
...
def load_protocol(expected_sha256: str) -> dict[str, Any]:
    ...
    if (
        value.get("schema")
        != "chacha20-round20-w52-deduplicated-reader-portfolio-recovery-a452-protocol-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("design_sha256") != DESIGN_SHA256
        or value.get("A451_result_sha256") != A451_RESULT_SHA256
        or value.get("A451_result_commitment_sha256")
        != A451_RESULT_COMMITMENT_SHA256
        or value.get("A426_protocol_sha256") != A426_PROTOCOL_SHA256
        or schedule.get("algorithm")
        != "A451_deduplicated_fixed_slot_round_robin_min_rank_pair_stream"
        ...
        or schedule.get("A451_result_commitment_sha256")
        != A451_RESULT_COMMITMENT_SHA256
        or schedule.get("A451_fusion_sha256") != A451_FUSION_SHA256
        or schedule.get("A451_guarantee_sha256") != A451_GUARANTEE_SHA256
        ...
        != [f"deduplicated_portfolio_wave_{index}" for index in range(WORKERS)]
        or boundary.get("A451_schedule_frozen_before_A452_challenge_binding")
        is not True
        or boundary.get("A434_qualification_content_read_at_A452_protocol_freeze")
        is not False
        ...
        is not False
        or boundary.get("A450_candidate_progress_or_result_read") is not False
        or boundary.get("A452_candidate_or_progress_available_at_protocol_freeze")
        is not False
        or boundary.get("A452_target_labels_used_for_schedule") != 0
        or boundary.get("A452_feature_refits") != 0
        or boundary.get("A452_model_refits") != 0
        ...
        raise RuntimeError("A452 protocol semantics differ")
    for row in value["anchors"].values():
        anchor(path_from_ref(row["path"]), row["sha256"])
    ...
    a426, _challenge = load_public_challenge()
    if (
        schedule["pair_stream_sha256"] != source["artifact"]["sha256"]
        ...
        raise RuntimeError("A452 source binding differs")
    unsigned = {
        key: item
        for key, item in value.items()
    ...
    if value.get("protocol_commitment_sha256") != canonical_sha256(unsigned):
        raise RuntimeError("A452 protocol commitment differs")
    return value
...
def worker_global_index(worker_index: int, zero_based_worker_step: int) -> int:
    ...
    if not 0 <= zero_based_worker_step < WORKER_TASKS:
        raise ValueError("A452 worker step differs")
...
def normalize_worker_row(row: Mapping[str, Any], worker_index: int) -> dict[str, Any]:
    value = dict(row)
    value["worker_role"] = f"deduplicated_portfolio_wave_{worker_index}"
...
def load_resume(
    *, worker_index: int, protocol_sha256: str, qualification_sha256: str
    ...
    if (
        value.get("schema")
        != "chacha20-round20-w52-deduplicated-reader-portfolio-recovery-a452-progress-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("worker_index") != worker_index
        or value.get("worker_role") != f"deduplicated_portfolio_wave_{worker_index}"
        ...
        raise RuntimeError("A452 progress fingerprint differs")
    ...
    if (
        ...
        raise RuntimeError("A452 resumable progress state differs")
...
def mark_peer_confirmed(
    ...
    if (
        ...
        raise RuntimeError("A452 peer progress fingerprint differs")
    if value.get("status") in {"running", "peer_confirmed"}:
        ...
        raise RuntimeError("A452 peer progress status differs")
...
def progress_snapshot(protocol_sha256: str, qualification_sha256: str) -> dict[str, Any]:
    ...
    for index in range(WORKERS):
        role = f"deduplicated_portfolio_wave_{index}"
        ...
        if (
            ...
            raise RuntimeError("A452 progress snapshot fingerprint differs")
...
def build_causal(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    writer = CausalWriter(api_id="a452recover")
    writer._rules = []
    writer.add_rule(
        name="bounded_regret_stream_to_factual_model",
        description="Execute the exact A451 non-factorized bounded-regret pair stream as eight disjoint complete-cell streams.",
        pattern=["A451_nonfactorized_W52_recovery_stream_ready"],
        conclusion="A452_sole_factual_W52_model",
    ...
    writer.add_rule(
        ...
        pattern=["A452_sole_factual_W52_model"],
        conclusion=f"A452_{terminal}",
        confidence_modifier=1.0,
    )
    writer.add_triplet(
        trigger="A451:nonfactorized_W52_recovery_stream_ready",
        mechanism="eight_disjoint_memory_mapped_complete_cell_streams",
        outcome="A452:sole_factual_W52_model",
        confidence=1.0,
    ...
    writer.add_triplet(
        ...
        outcome=f"A452:{terminal}",
        confidence=1.0,
        source="materialized:A452_bounded_regret_W52_recovery_chain",
    ...
    writer.add_cluster(
        name="A452 deduplicated portfolio Reader W52 recovery",
        entities=[
            "A451:nonfactorized_W52_recovery_stream_ready",
            "A452:sole_factual_W52_model",
            f"A452:{terminal}",
        ],
    )
    writer.add_gap(
        subject=f"A452:{terminal}",
    ...
    if (
        ...
        raise RuntimeError("A452 authentic Causal reopen gate failed")
    return {
...
def finalize_confirmed_stop(
    ...
    if (
        stop.get("schema")
        != "chacha20-round20-w52-deduplicated-reader-portfolio-recovery-a452-confirmed-stop-v1"
    ...
    if not 1 <= unique_cells <= PAIR_CELLS:
        raise RuntimeError("A452 aggregate unique pair-cell count differs")
    ...
    if (
        ...
        raise RuntimeError("A452 discovery rank identity differs")
    ...
    for name in A451.COMPONENTS:
        row = a449["operator_schedules"][name]
        component_ranks[name] = factorized_pair_global_index(
            prefix,
            off_axis,
            row["prefix_order"],
            row["off_axis_order"],
    ...
    if fused_rank > len(A451.COMPONENTS) * minimum_component_rank:
        raise RuntimeError("A452 realized A451 rank guarantee failed")
    rank_analysis = {
        "prefix12": prefix,
        "off_axis12": off_axis,
        "deduplicated_reader_portfolio_global_pair_rank_one_based": fused_rank,
        ...
        "exact_threefold_rank_guarantee_satisfied": fused_rank
        <= len(A451.COMPONENTS) * minimum_component_rank,
    ...
    evidence_stage = (
        "FULLROUND_R20_BOUNDED_REGRET_PORTFOLIO_W52_STRICT_SUBSET_RECOVERY_CONFIRMED"
        if strict_subset
        else "FULLROUND_R20_BOUNDED_REGRET_PORTFOLIO_W52_COMPLETE_DOMAIN_RECOVERY_CONFIRMED"
    )
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w52-deduplicated-reader-portfolio-recovery-a452-result-v1",
        "attempt_id": ATTEMPT_ID,
        ...
        "A451_result_sha256": A451_RESULT_SHA256,
        "A451_result_commitment_sha256": A451_RESULT_COMMITMENT_SHA256,
        "A451_pair_stream_sha256": A451_PAIR_STREAM_SHA256,
    ...
    atomic_bytes(
        REPORT,
        (
            "# A452 — deduplicated portfolio Reader full-round ChaCha20 W52 recovery\n\n"
            f"Evidence stage: **{evidence_stage}**\n\n"
            "- Frozen schedule: **A451 target-blind non-factorized bounded-regret reader portfolio**\n"
            ...
            f"- Deduplicated portfolio global pair rank: **{fused_rank:,}**\n"
            ...
            f"- Deduplicated portfolio gain over A442: **{rank_analysis['fused_gain_over_A442_bits']:.9f} bits**\n"
...
def recover_worker(
    *,
    ...
    role = f"deduplicated_portfolio_wave_{worker_index}"
    ...
    def write_progress(row: Mapping[str, Any]) -> None:
        ...
        atomic_json(
            ...
            {
                "schema": "chacha20-round20-w52-deduplicated-reader-portfolio-recovery-a452-progress-v1",
                "attempt_id": ATTEMPT_ID,
                "worker_index": worker_index,
    ...
    if discovery["status"] in {"peer_confirmed", "worker_exhausted"}:
        ...
        if discovery["status"] == "worker_exhausted":
            ...
            if exhausted == WORKERS:
                raise RuntimeError("A452 all disjoint workers exhausted without a model")
        return discovery
    ...
    if confirmation.get("all_blocks_match") is not True:
        raise RuntimeError("A452 dual-reference eight-block confirmation failed")
    if discovery.get("control_filter_candidates") != []:
        raise RuntimeError("A452 matched control produced a candidate")
    if STOP.exists():
        return mark_peer_confirmed(
            expected_protocol_sha256,
            expected_a434_qualification_sha256,
    ...
    atomic_json(
        STOP,
        {
            "schema": "chacha20-round20-w52-deduplicated-reader-portfolio-recovery-a452-confirmed-stop-v1",
            "attempt_id": ATTEMPT_ID,
            "protocol_sha256": expected_protocol_sha256,
            "A434_qualification_sha256": expected_a434_qualification_sha256,
            "A451_result_commitment_sha256": A451_RESULT_COMMITMENT_SHA256,
