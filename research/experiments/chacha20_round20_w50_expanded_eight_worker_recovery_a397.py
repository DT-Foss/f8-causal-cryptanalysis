#!/usr/bin/env python3
"""A397: execute A396's eight exact static W50 worker lists."""
...
DESIGN = (
    CONFIGS / "chacha20_round20_w50_expanded_eight_worker_recovery_a397_design_v1.json"
)
IMPLEMENTATION = (
    CONFIGS
    / "chacha20_round20_w50_expanded_eight_worker_recovery_a397_implementation_v1.json"
)
PROTOCOL = (
    CONFIGS / "chacha20_round20_w50_expanded_eight_worker_recovery_a397_v1.json"
)
RESULT = RESULTS / "chacha20_round20_w50_expanded_eight_worker_recovery_a397_v1.json"
CAUSAL = RESULT.with_suffix(".causal")
REPORT = RESULT.with_suffix(".md")
TEST = ROOT / "tests/test_chacha20_round20_w50_expanded_eight_worker_recovery_a397.py"
REPRO = (
    ROOT / "scripts/reproduce_chacha20_round20_w50_expanded_eight_worker_recovery_a397.sh"
)

A395_RUNNER = (
    RESEARCH
    / "experiments/chacha20_round20_w50_balanced_work_stealing_recovery_a395.py"
)
A395_PROTOCOL = (
...
A396_CAUSAL = A396_RESULT.with_suffix(".causal")

ATTEMPT_ID = "A397"
DESIGN_SHA256 = "5381a8bccf4498367a331054c314f1a7f681a1f1107a4e6c74eee15260c6520f"
A395_RUNNER_SHA256 = "43f65d648649de9920bacc99494d8a7ea7daad68afad9a68b87a67ab6f97c727"
A395_PROTOCOL_SHA256 = "bdf504b8fccf4f75dc0cab8fd9fb7bcf98e666b0b322fb211ae7f1be24c2eecd"
A396_RESULT_SHA256 = "0dd39b595bd111c88b1c94c325c1039a041211e8e11e1e79f71c6470cc8edff8"
A396_CAUSAL_SHA256 = "f8187d11c2a8bce9e8e7f8d43a160b1a205b73fa5508190091b46407ed6f0939"
...
ROLE_SPARE = "A396_spare_longest_remaining"
ROLES = (*SOURCE_ROLES, ROLE_SPARE)
SLUG_TO_ROLE = {
    "a379": "A379_target_free",
    "a380": "A380_target_conditioned",
    "wide": "wide_vote",
    "sparse": "sparse_reciprocal",
    "quantile": "broad_quantile",
    "intersection": "broad_intersection",
...
def load_module(path: Path, name: str) -> Any:
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot import A397 dependency {path}")
...
A395 = load_module(A395_RUNNER, "a397_a395")
file_sha256 = A395.file_sha256
canonical_sha256 = A395.canonical_sha256
atomic_json = A395.atomic_json
atomic_bytes = A395.atomic_bytes
relative = A395.relative
path_from_ref = A395.path_from_ref
anchor = A395.anchor


def progress_path(worker: str) -> Path:
    if worker not in ROLE_TO_SLUG:
        raise ValueError(f"A397 unknown worker role {worker}")
    return RESULTS / (
        "chacha20_round20_w50_expanded_eight_worker_recovery_"
        f"a397_{ROLE_TO_SLUG[worker]}_progress_v1.json"
    )


def worker_order_sha256(values: Sequence[int]) -> str:
    return A395.A393.lane_sha256([int(value) for value in values])
...
def exact_worker_schedule(
    ...
    if worker not in EXPECTED_WORKER_TASKS:
        raise ValueError(f"A397 unknown worker {worker}")
    ...
    if (
        ...
        raise ValueError(f"A397 {worker} schedule geometry differs")
    for index, (cell, row) in enumerate(zip(order, rows, strict=True), 1):
        ...
        if (
            ...
            or owner not in SOURCE_ROLES
            or int(row.get("owner_queue_position_one_based", 0)) < 1
            or row.get("stolen") is not (owner != worker)
        ):
            raise ValueError(f"A397 {worker} task semantics differ at step {index}")
    return order, rows


def load_design() -> dict[str, Any]:
    if file_sha256(DESIGN) != DESIGN_SHA256:
        raise RuntimeError("A397 design hash differs")
    ...
    if (
        value.get("schema")
        != "chacha20-round20-w50-expanded-eight-worker-recovery-a397-design-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("design_state")
        != "frozen_after_A396_exact_scheduler_before_any_A397_candidate_progress_or_result"
        ...
        or schedule.get("pointwise_A394_regressions") != 0
        or launch.get("first_available_slot_worker") != ROLE_DIRECT
        ...
        != 0
        or boundary.get(
            "A345_A353_A358_A374_A377_A381_A383_A387_progress_or_filter_outcomes_consumed"
        )
        is not False
        or boundary.get("A389_A391_A393_A395_progress_or_filter_outcomes_consumed")
        is not False
    ):
        raise RuntimeError("A397 frozen design semantics differ")
...
def load_schedule() -> tuple[
    ...
    anchor(A396_CAUSAL, A396_CAUSAL_SHA256)
    value = json.loads(A396_RESULT.read_bytes())
    ...
    if (
        ...
        or proof.get("A396_slower_than_A394_cells") != 0
    ):
        raise RuntimeError("A397 A396 schedule semantics differ")
    ...
    for worker in ROLES:
        ...
        if (
            ...
            raise RuntimeError(f"A397 {worker} schedule bytes differ")
    ...
    if (
        ...
        raise RuntimeError("A397 static worker cover differs")
    owner_positions: dict[str, list[int]] = {role: [] for role in SOURCE_ROLES}
    for epoch in range(1, STATIC_EPOCHS + 1):
        for worker in ROLES:
            ...
            if (
                ...
                raise RuntimeError("A397 cell-indexed schedule differs")
            owner_positions[row["owner_queue_role"]].append(
                int(row["owner_queue_position_one_based"])
            )
    for owner, positions in owner_positions.items():
        if positions != list(range(1, len(positions) + 1)):
            raise RuntimeError(f"A397 {owner} owner queue order differs")
    return value, orders, tasks


def assert_pre_execution() -> None:
    if RESULT.exists() or CAUSAL.exists() or REPORT.exists():
        raise RuntimeError("A397 freeze must precede result artifacts")
    if any(progress_path(worker).exists() for worker in ROLES):
        raise RuntimeError("A397 freeze must precede every worker progress artifact")


def freeze_implementation() -> dict[str, Any]:
    if IMPLEMENTATION.exists() or PROTOCOL.exists():
        raise FileExistsError("A397 implementation or protocol already exists")
    assert_pre_execution()
    design = load_design()
    schedule, orders, tasks = load_schedule()
    base = A395.load_protocol(A395_PROTOCOL_SHA256)
    qualification = A395.A393.A389.A385.load_a384_qualification(
    ...
    if (
        ...
        raise RuntimeError("A397 challenge or W50 qualification differs")
    if not TEST.exists() or not REPRO.exists():
        raise FileNotFoundError("A397 test and reproducer must precede freeze")
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w50-expanded-eight-worker-recovery-a397-implementation-v1",
        "attempt_id": ATTEMPT_ID,
        "commitment_state": "frozen_before_any_A397_candidate_progress_or_result",
        "design_sha256": DESIGN_SHA256,
        "execution_contract": design["execution_contract"],
        "launch_contract": design["launch_contract"],
        "A396_schedule_commitment_sha256": schedule[
        ...
        "A397_candidate_or_progress_available_at_implementation_freeze": False,
        "anchors": {
            "design": anchor(DESIGN, DESIGN_SHA256),
            "A395_runner": anchor(A395_RUNNER, A395_RUNNER_SHA256),
            "A395_protocol": anchor(A395_PROTOCOL, A395_PROTOCOL_SHA256),
            "A396_result": anchor(A396_RESULT, A396_RESULT_SHA256),
            "A396_causal": anchor(A396_CAUSAL, A396_CAUSAL_SHA256),
            "A384_qualification": anchor(
                A395.A393.A389.A384_QUALIFICATION, A384_QUALIFICATION_SHA256
            ),
            "A385_protocol": anchor(
                A395.A393.A389.A385_PROTOCOL, A385_PROTOCOL_SHA256
            ),
...
def load_implementation(expected_sha256: str) -> dict[str, Any]:
    if file_sha256(IMPLEMENTATION) != expected_sha256:
        raise RuntimeError("A397 implementation hash differs")
    value = json.loads(IMPLEMENTATION.read_bytes())
    if (
        value.get("schema")
        != "chacha20-round20-w50-expanded-eight-worker-recovery-a397-implementation-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("design_sha256") != DESIGN_SHA256
        or value.get(
            "A397_candidate_or_progress_available_at_implementation_freeze"
        )
        is not False
    ):
        raise RuntimeError("A397 implementation semantics differ")
    ...
    if value.get("implementation_commitment_sha256") != canonical_sha256(unsigned):
        raise RuntimeError("A397 implementation commitment differs")
    return value


def freeze_protocol(*, expected_implementation_sha256: str) -> dict[str, Any]:
    if PROTOCOL.exists():
        raise FileExistsError("A397 protocol already exists")
    assert_pre_execution()
    implementation = load_implementation(expected_implementation_sha256)
    schedule, orders, tasks = load_schedule()
    base = A395.load_protocol(A395_PROTOCOL_SHA256)
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w50-expanded-eight-worker-recovery-a397-protocol-v1",
        "attempt_id": ATTEMPT_ID,
        "protocol_state": "eight_static_A396_workers_bound_before_any_A397_candidate_progress_or_result",
        "design_sha256": DESIGN_SHA256,
        ...
        "A396_result_sha256": A396_RESULT_SHA256,
        "A396_schedule_commitment_sha256": schedule[
            "schedule_commitment_sha256"
        ],
        ...
        "information_boundary": {
            "A397_candidate_or_progress_available_at_protocol_freeze": False,
            "A396_static_schedules_frozen_before_A397_execution": True,
    ...
    payload["protocol_commitment_sha256"] = canonical_sha256(
        {
            ...
            "A396_schedule_commitment_sha256": payload[
                "A396_schedule_commitment_sha256"
            ],
...
def load_protocol(expected_sha256: str) -> dict[str, Any]:
    if file_sha256(PROTOCOL) != expected_sha256:
        raise RuntimeError("A397 protocol hash differs")
    value = json.loads(PROTOCOL.read_bytes())
    raw_orders = value.get("worker_cell_orders", {})
    raw_tasks = value.get("worker_tasks", {})
    if set(raw_orders) != set(ROLES) or set(raw_tasks) != set(ROLES):
        raise RuntimeError("A397 protocol worker role set differs")
    ...
    for worker in ROLES:
        ...
        if (
            ...
            raise RuntimeError(f"A397 protocol {worker} schedule differs")
    flattened = [cell for worker in ROLES for cell in orders[worker]]
    boundary = value.get("information_boundary", {})
    if (
        value.get("schema")
        != "chacha20-round20-w50-expanded-eight-worker-recovery-a397-protocol-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or tuple(value.get("worker_role_order", [])) != ROLES
        or value.get("A396_schedule_commitment_sha256")
        != source["schedule_commitment_sha256"]
        or len(flattened) != CELLS
        or len(set(flattened)) != CELLS
        or set(flattened) != set(range(CELLS))
        or boundary.get("A397_candidate_or_progress_available_at_protocol_freeze")
        is not False
        or boundary.get("target_labels_used_for_schedule_construction") != 0
        or boundary.get("reader_refits") != 0
        or boundary.get(
            "all_live_or_queued_recovery_progress_filter_results_or_true_prefixes_consumed"
        )
        is not False
        or canonical_sha256(value.get("public_challenge"))
        != value.get("public_challenge_sha256")
    ):
        raise RuntimeError("A397 protocol semantics differ")
    for row in value["anchors"].values():
        anchor(path_from_ref(row["path"]), row["sha256"])
    load_implementation(value["implementation_sha256"])
    A395.A393.A389.A385.validate_challenge(value["public_challenge"])
    expected_commitment = canonical_sha256(
        {
            ...
            "A396_schedule_commitment_sha256": value[
                "A396_schedule_commitment_sha256"
            ],
    ...
    if value.get("protocol_commitment_sha256") != expected_commitment:
        raise RuntimeError("A397 protocol commitment differs")
    return value
...
def load_resume(
    *,
    ...
    if (
        value.get("schema")
        != "chacha20-round20-w50-expanded-eight-worker-recovery-a397-progress-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        ...
        raise RuntimeError("A397 progress fingerprint differs")
    ...
    if (
        ...
        raise RuntimeError("A397 resumable progress state differs")
...
def ordered_worker_discovery(
    *,
    ...
    if not 0 <= start_group <= len(order):
        raise ValueError("A397 resume group lies outside worker schedule")
    ...
    try:
        for worker_index in range(start_group, len(order)):
            ...
            if worker_index == start_group or worker_index % HOST_REFRESH_GROUPS == 0:
                ...
                host = host_factory()
                host_instances += 1
            observed = A395.A393.A389.A384.filter_complete_prefix(
                host=host,
                challenge=challenge,
                prefix=prefix,
                target=target,
            ...
            if controls:
                raise RuntimeError("A397 matched control produced a candidate")
            progress = {
            ...
            if not factual:
                ...
                continue
            if len(factual) != 1:
                raise RuntimeError("A397 complete W50 group produced multiple filters")
            candidate = factual[0]
            if ((candidate >> WORD0_SUFFIX_BITS) & (CELLS - 1)) != prefix:
                raise RuntimeError("A397 candidate prefix differs")
            decoded = A395.A393.A389.A384.decode_assignment(candidate)
            found = {
...
def progress_snapshot(protocol: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for worker in ROLES:
        ...
        if (
            ...
            raise RuntimeError("A397 progress snapshot fingerprint differs")
    ...
    return {
        ...
        "prefix_sets_are_disjoint_by_A396_construction": True,
    }
...
def rank_panel(
    ...
    if source["cell_worker_role"][prefix] != worker:
        raise RuntimeError("A397 confirmed prefix worker differs")
    task = protocol["worker_tasks"][worker][epoch - 1]
    row = source["per_cell_proof"][prefix]
    if int(task["cell"]) != prefix or int(row["cell"]) != prefix:
        raise RuntimeError("A397 confirmed task/proof lookup differs")
    return {
        **row,
        "A396_worker_role": worker,
        "A396_worker_step_one_based": int(task["worker_step_one_based"]),
        "A396_owner_queue_role": task["owner_queue_role"],
        "A396_owner_queue_position_one_based": int(
            task["owner_queue_position_one_based"]
        ),
        "A396_task_stolen": task["stolen"],
        "exact_depth_chain_holds": (
            epoch
            <= int(row["owner_lane_depth_one_based"])
            <= int(row["best_source_rank_one_based"])
            and epoch <= int(row["A394_epoch_one_based"])
        ),
...
def build_causal(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    terminal = "A397:confirmed_expanded_eight_worker_W50_recovery"
    writer = CausalWriter(api_id="a397w50")
    writer._rules = []
    writer.add_rule(
        name="expanded_schedule_and_engine_to_model",
        description="A396's eight disjoint static worker lists execute complete qualified W50 groups with one shared confirmed-success stop.",
        pattern=["A396_exact_eight_worker_W50_schedule", "A384_exact_W50_group_engine"],
        conclusion="A397_sole_factual_W50_model",
        confidence_modifier=1.0,
    )
    writer.add_rule(
        name="model_to_confirmed_expanded_recovery",
        description="The sole factual model passes the matched control and dual independent eight-block confirmation.",
        pattern=["A397_sole_factual_W50_model"],
        conclusion="A397_confirmed_expanded_eight_worker_W50_recovery",
        confidence_modifier=1.0,
    )
    writer.add_triplet(
        trigger="A396:exact_eight_worker_W50_schedule",
        mechanism="qualified_eight_worker_complete_group_search_with_shared_stop",
        outcome="A397:sole_factual_W50_model",
        confidence=1.0,
        ...
        domain="fresh full-round ChaCha20 W50 expanded parallel recovery",
        quality_score=1.0,
    )
    writer.add_triplet(
        trigger="A397:sole_factual_W50_model",
        ...
        domain="confirmed full-round ChaCha20 W50 eight-worker recovery",
        quality_score=1.0,
    )
    writer.add_triplet(
        trigger="A396:exact_eight_worker_W50_schedule",
        mechanism="materialized_expanded_search_and_confirmation_closure",
        outcome=terminal,
        confidence=1.0,
        source="materialized:A397_expanded_W50_recovery_chain",
    ...
    writer.add_cluster(
        name="A397 expanded eight-worker W50 recovery",
        entities=[
            "A396:exact_eight_worker_W50_schedule",
            "A397:sole_factual_W50_model",
            terminal,
        ],
    )
    writer.add_gap(
        ...
        expected_object_type="W51_expanded_reader_parallel_transfer",
        confidence=1.0,
        suggested_queries=[
            "Transfer the expanded primitive-Reader scheduler after W51 engine qualification and a fresh W51 Direct12 order."
        ],
    )
    ...
    if (
        ...
        raise RuntimeError("A397 authentic Causal reopen gate failed")
    return {
...
def recover_worker(*, worker: str, expected_protocol_sha256: str) -> dict[str, Any]:
    ...
    engine_protocol = A395.A393.A389.A384.load_protocol(
        A395.A393.A389.A384_PROTOCOL_SHA256
    )
    ...
    def host_factory() -> Any:
        return A395.A393.A389.A384.A378.A371.A346.A324.A311.A307.A304.GroupedMetalHost(
            executable,
            A395.A393.A389.A384.initial_for_slab(challenge, 0),
            placeholder,
            placeholder,
        )
    ...
    def write_progress(row: Mapping[str, Any]) -> None:
        atomic_json(
            ...
            {
                "schema": "chacha20-round20-w50-expanded-eight-worker-recovery-a397-progress-v1",
                "attempt_id": ATTEMPT_ID,
                "worker_role": worker,
                "worker_slug": ROLE_TO_SLUG[worker],
    ...
    if discovery["status"] in {"peer_confirmed", "worker_exhausted"}:
        if discovery["status"] == "worker_exhausted":
            ...
            if exhausted == len(ROLES) and not RESULT.exists():
                raise RuntimeError("A397 all static workers exhausted without a model")
        return discovery
    candidate = int(discovery["candidate"])
    confirmation = A395.A393.A389.A385.confirm(challenge, candidate)
    if confirmation["all_blocks_match"] is not True:
        raise RuntimeError("A397 dual independent confirmation failed")
    ranks = rank_panel(int(discovery["prefix12"]), worker, protocol)
    if (
        int(ranks["A396_epoch_one_based"])
        != discovery["static_schedule_epoch"]
        or ranks["A396_worker_step_one_based"]
        != discovery["executed_worker_prefix_groups"]
        or ranks["exact_depth_chain_holds"] is not True
    ):
        raise RuntimeError("A397 discovery depth differs from A396 schedule")
    ...
    if not 1 <= unique_groups <= CELLS:
        raise RuntimeError("A397 aggregate unique group count differs")
    ...
    evidence_stage = (
        "FULLROUND_R20_EXPANDED_EIGHT_WORKER_W50_STRICT_SUBSET_RECOVERY_CONFIRMED"
        if strict_subset
        else "FULLROUND_R20_EXPANDED_EIGHT_WORKER_W50_COMPLETE_DOMAIN_RECOVERY_CONFIRMED"
    )
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w50-expanded-eight-worker-recovery-a397-result-v1",
        "attempt_id": ATTEMPT_ID,
        ...
        "A396_result_sha256": protocol["A396_result_sha256"],
        "A396_schedule_commitment_sha256": protocol[
            "A396_schedule_commitment_sha256"
        ],
    ...
    atomic_bytes(
        REPORT,
        (
            "# A397 — expanded eight-worker full-round ChaCha20 W50 recovery\n\n"
            f"Evidence stage: **{evidence_stage}**\n\n"
            f"- Winning worker: **{worker}**\n"
            f"- Static schedule epoch: **{ranks['A396_epoch_one_based']} / {STATIC_EPOCHS}**\n"
            ...
            "- Duplicate A397 prefix groups: **zero by construction**\n"
