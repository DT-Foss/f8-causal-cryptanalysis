#!/usr/bin/env python3
"""A387: execute A385's pre-target order over the complete fresh ChaCha20 W50 domain."""
...
DESIGN = CONFIGS / "chacha20_round20_w50_pretarget_factor6_recovery_a387_design_v1.json"
IMPLEMENTATION = CONFIGS / "chacha20_round20_w50_pretarget_factor6_recovery_a387_implementation_v1.json"
PROTOCOL = CONFIGS / "chacha20_round20_w50_pretarget_factor6_recovery_a387_v1.json"
PROGRESS = RESULTS / "chacha20_round20_w50_pretarget_factor6_recovery_a387_progress_v1.json"
RESULT = RESULTS / "chacha20_round20_w50_pretarget_factor6_recovery_a387_v1.json"
CAUSAL = RESULT.with_suffix(".causal")
REPORT = RESULT.with_suffix(".md")
TEST = ROOT / "tests/test_chacha20_round20_w50_pretarget_factor6_recovery_a387.py"
REPRO = ROOT / "scripts/reproduce_chacha20_round20_w50_pretarget_factor6_recovery_a387.sh"
...
ATTEMPT_ID = "A387"
DESIGN_SHA256 = "a35f64d5963dfb7150a07a0d8bfa9687c45469b499f2610c1b3e1c3efab7e3df"
...
SELECTED_OPERATOR = "A385_byte_identical_A382_six_view_factor6_pre_target_W50_transfer"
...
def load_module(path: Path, name: str) -> Any:
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot import A387 dependency {path}")
...
A385 = load_module(A385_RUNNER, "a387_a385")
A384 = A385.A384
...
def exact_order(values: Sequence[int], label: str) -> list[int]:
    ...
    if len(order) != CELLS or set(order) != set(range(CELLS)):
        raise ValueError(f"A387 {label} is not an exact 4,096-cell order")
    return order
...
def load_design() -> dict[str, Any]:
    if file_sha256(DESIGN) != DESIGN_SHA256:
        raise RuntimeError("A387 design hash differs")
    value = json.loads(DESIGN.read_bytes())
    execution = value.get("execution_contract", {})
    boundary = value.get("information_boundary", {})
    order = value.get("order_contract", {})
    if (
        value.get("schema") != "chacha20-round20-w50-pretarget-factor6-recovery-a387-design-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("design_state")
        != "frozen_after_A384_exact_W50_qualification_and_A385_fresh_protocol_before_any_A387_candidate_prefix_or_result"
        ...
        or order.get("executed_order_uint16be_sha256") != A385_ORDER_UINT16BE_SHA256
        ...
        or boundary.get("A387_candidate_or_prefix_available_at_design_freeze") is not False
    ):
        raise RuntimeError("A387 frozen design semantics differ")
...
def assert_pre_execution() -> None:
    if PROGRESS.exists() or RESULT.exists() or CAUSAL.exists() or REPORT.exists():
        raise RuntimeError("A387 frozen artifacts must precede every W50 candidate and result")
...
def load_sources() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    order = A385.load_order()
    qualification = A385.load_a384_qualification(A384_QUALIFICATION_SHA256)
    anchor(A386_ORDER, A386_ORDER_SHA256)
    anchor(A386_CAUSAL, A386_CAUSAL_SHA256)
    comparison = json.loads(A386_ORDER.read_bytes())
    selected = exact_order(protocol.get("selected_order", []), "A385 protocol order")
    copied = exact_order(order.get("selected_order", []), "A385 order")
    comparison_order = exact_order(comparison.get("selected_order", []), "A386 comparison")
    if (
        ...
        or protocol.get("information_boundary", {}).get(
            "A385_target_labels_used_for_order_construction"
        )
        != 0
        or selected != copied
        or order_sha256(selected) != A385_ORDER_UINT16BE_SHA256
        or comparison.get("W49_target_labels_used") != 0
        or comparison.get("W49_candidate_or_filter_outcomes_used") != 0
        or order_sha256(comparison_order) != A386_ORDER_UINT16BE_SHA256
        or qualification.get("matched_control_empty") is not True
        or qualification.get("complete_group_gate", {}).get("logical_candidates")
        != GROUP_SIZE
    ):
        raise RuntimeError("A387 source semantics differ")
    return protocol, order, qualification, comparison


def freeze_implementation() -> dict[str, Any]:
    if any(path.exists() for path in (IMPLEMENTATION, PROTOCOL, PROGRESS, RESULT, CAUSAL, REPORT)):
        raise FileExistsError("A387 implementation or execution artifact already exists")
    assert_pre_execution()
    load_design()
    load_sources()
    if not TEST.exists() or not REPRO.exists():
        raise FileNotFoundError("A387 test and reproducer must precede implementation freeze")
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w50-pretarget-factor6-recovery-a387-implementation-v1",
        "attempt_id": ATTEMPT_ID,
        "commitment_state": "frozen_after_A384_qualification_A385_protocol_and_A386_comparator_before_A387_candidate_prefix_or_result",
        "design_sha256": DESIGN_SHA256,
        "selected_operator": SELECTED_OPERATOR,
        "A387_candidate_or_prefix_available_at_implementation_freeze": False,
        "anchors": {
            ...
            "A386_causal": anchor(A386_CAUSAL, A386_CAUSAL_SHA256),
...
def load_implementation(expected_sha256: str) -> dict[str, Any]:
    if file_sha256(IMPLEMENTATION) != expected_sha256:
        raise RuntimeError("A387 implementation hash differs")
    value = json.loads(IMPLEMENTATION.read_bytes())
    if (
        value.get("schema")
        != "chacha20-round20-w50-pretarget-factor6-recovery-a387-implementation-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("design_sha256") != DESIGN_SHA256
        or value.get("selected_operator") != SELECTED_OPERATOR
        or value.get("A387_candidate_or_prefix_available_at_implementation_freeze")
        is not False
    ):
        raise RuntimeError("A387 implementation semantics differ")
    for row in value["anchors"].values():
        anchor(path_from_ref(row["path"]), row["sha256"])
    unsigned = {key: item for key, item in value.items() if key != "implementation_commitment_sha256"}
    if value.get("implementation_commitment_sha256") != canonical_sha256(unsigned):
        raise RuntimeError("A387 implementation commitment differs")
    return value
...
def freeze_protocol(*, expected_implementation_sha256: str) -> dict[str, Any]:
    if any(path.exists() for path in (PROTOCOL, PROGRESS, RESULT, CAUSAL, REPORT)):
        raise FileExistsError("A387 protocol or execution artifact already exists")
    assert_pre_execution()
    implementation = load_implementation(expected_implementation_sha256)
    source, order, qualification, comparison = load_sources()
    selected = exact_order(source["selected_order"], "protocol selected")
    comparison_order = exact_order(comparison["selected_order"], "protocol comparison")
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w50-pretarget-factor6-recovery-a387-protocol-v1",
        "attempt_id": ATTEMPT_ID,
        "protocol_state": "A385_pre_target_order_bound_before_any_A387_candidate_prefix_or_result",
        "design_sha256": DESIGN_SHA256,
        ...
        "A385_order_sha256": A385_ORDER_SHA256,
        "A385_order_commitment_sha256": order["order_commitment_sha256"],
        "A386_comparison_order_sha256": A386_ORDER_SHA256,
        ...
        "comparison_order_uint16be_sha256": order_sha256(comparison_order),
        "comparison_order": comparison_order,
        "source_pointwise_factor6_proof": source["source_pointwise_factor6_proof"],
        "comparison_outer_factor3_proof": comparison["outer_factor3_proof"],
        "execution_contract": load_design()["execution_contract"],
        "information_boundary": {
            "A387_candidate_or_prefix_available_at_protocol_freeze": False,
            "A385_order_frozen_before_W50_challenge": True,
            "A386_comparison_order_frozen_before_W50_challenge": True,
    ...
    payload["protocol_commitment_sha256"] = canonical_sha256(
        {
            ...
            "comparison_order_uint16be_sha256": payload[
                "comparison_order_uint16be_sha256"
            ],
...
def load_protocol(expected_sha256: str) -> dict[str, Any]:
    if file_sha256(PROTOCOL) != expected_sha256:
        raise RuntimeError("A387 protocol hash differs")
    value = json.loads(PROTOCOL.read_bytes())
    selected = exact_order(value.get("selected_order", []), "stored selected")
    comparison = exact_order(value.get("comparison_order", []), "stored comparison")
    if (
        value.get("schema")
        != "chacha20-round20-w50-pretarget-factor6-recovery-a387-protocol-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("selected_operator") != SELECTED_OPERATOR
        or value.get("selected_order_uint16be_sha256") != order_sha256(selected)
        or value.get("comparison_order_uint16be_sha256") != order_sha256(comparison)
        or value.get("information_boundary", {}).get(
            "A387_candidate_or_prefix_available_at_protocol_freeze"
        )
        is not False
        or value.get("information_boundary", {}).get(
        ...
        raise RuntimeError("A387 protocol semantics differ")
    for row in value["anchors"].values():
        anchor(path_from_ref(row["path"]), row["sha256"])
...
def load_resume(
    *, protocol_sha256: str, order_sha: str, qualification_sha256: str
    ...
    if (
        value.get("schema")
        != "chacha20-round20-w50-pretarget-factor6-recovery-a387-progress-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        ...
        raise RuntimeError("A387 progress fingerprint differs")
    ...
    if not 0 <= completed < CELLS or value.get("factual_filter_candidates") != 0:
        raise RuntimeError("A387 resumable progress state differs")
    return (
        completed,
...
def ordered_discovery(
    *,
    ...
    if not 0 <= start_group < CELLS:
        raise ValueError("A387 resume group lies outside the prefix cover")
    ...
    try:
        for group_index in range(start_group, CELLS):
            ...
            if controls:
                raise RuntimeError("A387 matched control produced a candidate")
            if not factual:
                ...
                continue
            if len(factual) != 1:
                raise RuntimeError("A387 complete W50 group produced multiple filters")
            candidate = factual[0]
            if ((candidate >> WORD0_SUFFIX_BITS) & (CELLS - 1)) != prefix:
                raise RuntimeError("A387 candidate prefix differs")
        ...
        if host is not None:
            host.close()
    raise RuntimeError("A387 exact frozen order exhausted without a factual filter")


def rank_panel(prefix: int, protocol: Mapping[str, Any]) -> dict[str, Any]:
    selected_rank = rank_vector(protocol["selected_order"])[prefix]
    comparison_rank = rank_vector(protocol["comparison_order"])[prefix]
    return {
        "prefix12": prefix,
        "prefix12_hex": f"{prefix:03x}",
        "A385_executed_factor6_rank_one_based": selected_rank,
        "A386_calibrated_factor3_rank_one_based": comparison_rank,
        "selected_gain_bits_vs_complete_domain": math.log2(CELLS / selected_rank),
        "selected_domain_reduction_factor": CELLS / selected_rank,
        "comparison_rank_ratio_to_selected": comparison_rank / selected_rank,
        "comparison_rank_gain_bits_to_selected": math.log2(selected_rank / comparison_rank),
...
def build_causal(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    terminal = "A387:confirmed_pre_target_W50_recovery"
    writer = CausalWriter(api_id="a387w50")
    writer._rules = []
    writer.add_rule(
        name="pretarget_order_and_W50_engine_to_model",
        description="The order frozen before the fresh W50 challenge executes complete one-hundred-twenty-eight-slab groups until a sole factual model appears.",
        pattern=["A385_frozen_pre_target_W50_order", "A384_exact_W50_group_engine"],
        conclusion="A387_sole_factual_W50_model",
    ...
    writer.add_rule(
        ...
        pattern=["A387_sole_factual_W50_model"],
        conclusion="A387_confirmed_pre_target_W50_recovery",
        confidence_modifier=1.0,
    )
    writer.add_triplet(
        trigger="A385:pre_target_W50_factor6_order",
        mechanism="qualified_ordered_complete_onehundredtwentyeight_slab_search",
        outcome="A387:sole_factual_W50_model",
        confidence=1.0,
        source=payload["execution_sha256"],
        quantification=json.dumps(payload["discovery"], sort_keys=True),
        evidence=json.dumps(payload["rank_analysis"], sort_keys=True),
        domain="fresh full-round ChaCha20 W50 pre-target recovery",
        quality_score=1.0,
    )
    writer.add_triplet(
        trigger="A387:sole_factual_W50_model",
    ...
    writer.add_triplet(
        ...
        outcome=terminal,
        confidence=1.0,
        source="materialized:A387_W50_recovery_chain",
    ...
    writer.add_cluster(
        name="A387 pre-target W50 recovery",
        entities=[
            "A385:pre_target_W50_factor6_order",
            "A387:sole_factual_W50_model",
            terminal,
        ],
    )
    writer.add_gap(
        ...
        confidence=1.0,
        suggested_queries=[
            "Lift the exact engine to W51 or execute the independently frozen A386 comparison order on the same W50 challenge."
        ],
    )
    ...
    if (
        ...
        raise RuntimeError("A387 authentic Causal reopen gate failed")
    return {
...
def recover(*, expected_protocol_sha256: str) -> dict[str, Any]:
    if any(path.exists() for path in (RESULT, CAUSAL, REPORT)):
        raise FileExistsError("A387 result artifact already exists")
    ...
    def write_progress(row: Mapping[str, Any]) -> None:
        atomic_json(
            PROGRESS,
            {
                "schema": "chacha20-round20-w50-pretarget-factor6-recovery-a387-progress-v1",
                "attempt_id": ATTEMPT_ID,
    ...
    if confirmation["all_blocks_match"] is not True:
        raise RuntimeError("A387 dual independent confirmation failed")
    ranks = rank_panel(int(discovery["prefix12"]), protocol)
    if ranks["A385_executed_factor6_rank_one_based"] != discovery[
        "executed_prefix_groups"
    ]:
        raise RuntimeError("A387 discovery rank differs from selected order")
    strict_subset = discovery["executed_prefix_groups"] < CELLS
    evidence_stage = (
        "FULLROUND_R20_PRETARGET_W50_STRICT_SUBSET_RECOVERY_CONFIRMED"
        if strict_subset
        else "FULLROUND_R20_PRETARGET_W50_COMPLETE_DOMAIN_RECOVERY_CONFIRMED"
    )
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w50-pretarget-factor6-recovery-a387-result-v1",
        "attempt_id": ATTEMPT_ID,
        ...
        "A385_order_sha256": protocol["A385_order_sha256"],
        "A386_comparison_order_sha256": protocol[
            "A386_comparison_order_sha256"
        ],
        ...
        "source_pointwise_factor6_proof": protocol[
            "source_pointwise_factor6_proof"
        ],
        "comparison_outer_factor3_proof": protocol[
            "comparison_outer_factor3_proof"
        ],
        "qualification_gate": {
    ...
    atomic_bytes(
        REPORT,
        (
            "# A387 — pre-target full-round ChaCha20 W50 recovery\n\n"
            f"Evidence stage: **{evidence_stage}**\n\n"
            f"- W50 execution rank: **{ranks['A385_executed_factor6_rank_one_based']} / {CELLS}**\n"
            f"- A386 calibrated comparison rank: **{ranks['A386_calibrated_factor3_rank_one_based']} / {CELLS}**\n"
