#!/usr/bin/env python3
"""A426: execute the qualified A416 schedule on a fresh W52 challenge."""
...
STEM = "chacha20_round20_w52_a416_fresh_shared_stop_recovery_a426"
...
ATTEMPT_ID = "A426"
DESIGN_SHA256 = "ee46dca62d23252932dddf6eed7aaeea5359928e3b89c830cf593d6bd6132eaf"
...
def load_module(path: Path, name: str) -> Any:
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot import A426 dependency {path}")
...
A422 = load_module(A422_RUNNER, "a426_a422")
W43 = A422.W43
...
def progress_path(worker_index: int) -> Path:
    if not 0 <= worker_index < WORKERS:
        raise ValueError("A426 worker index differs")
...
def load_design() -> dict[str, Any]:
    if file_sha256(DESIGN) != DESIGN_SHA256:
        raise RuntimeError("A426 design hash differs")
    ...
    if (
        value.get("schema")
        != "chacha20-round20-w52-a416-fresh-shared-stop-recovery-a426-design-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("design_state")
        != "frozen_after_A416_external_qualification_and_A422_target_free_protocol_before_A422_qualification_any_A424_A425_outcome_or_any_A426_challenge_assignment_candidate_progress_or_filter_outcome"
        ...
        or transfer.get("parameter_refit_on_A426_target") is not False
        or fresh.get("materialize_only_after_complete_A426_implementation_freeze") is not True
        or fresh.get("secret_assignment_absent_from_protocol") is not True
        or boundary.get("A416_result_available_at_A426_design_freeze") is not True
        or boundary.get("A422_qualification_available_at_A426_design_freeze") is not False
        or boundary.get("A424_result_available_at_A426_design_freeze") is not False
        or boundary.get("A425_result_available_at_A426_design_freeze") is not False
        or boundary.get("A426_production_challenge_available_at_design_freeze") is not False
        or boundary.get("A426_target_assignment_available_at_design_freeze") is not False
        or boundary.get("A426_candidate_or_progress_available_at_design_freeze") is not False
        or boundary.get("A426_target_labels_used_for_schedule") != 0
        or boundary.get("prior_live_recovery_filter_outcomes_consumed") is not False
    ):
        raise RuntimeError("A426 frozen design semantics differ")
...
def validate_worker_schedule(
    *,
    ...
    if not 0 <= worker_index < WORKERS or roles[worker_index] != worker_role:
        raise ValueError("A426 worker identity differs")
    ...
    if (
        ...
        raise ValueError("A426 worker schedule geometry differs")
    for step, (cell, row) in enumerate(zip(order, rows, strict=True), 1):
        ...
        if (
            ...
            raise ValueError(
                f"A426 task semantics differ at worker {worker_index} step {step}"
            )
    return order, rows
...
def load_a416_schedule() -> tuple[
    ...
    if file_sha256(A416_RESULT) != A416_RESULT_SHA256:
        raise RuntimeError("A426 A416 result hash differs")
    ...
    if (
        ...
        raise RuntimeError("A426 A416 source semantics differ")
    ...
    if len(flat) != CELLS or len(set(flat)) != CELLS or set(flat) != set(range(CELLS)):
        raise RuntimeError("A426 A416 schedule cover differs")
...
def load_a422_qualification(expected_sha256: str) -> dict[str, Any]:
    if file_sha256(A422_QUALIFICATION) != expected_sha256:
        raise RuntimeError("A426 A422 qualification artifact hash differs")
    ...
    if (
        ...
        or value.get("evidence_stage")
        != "TARGET_FREE_COMPLETE_W52_GROUP_ENGINE_EXACTLY_QUALIFIED"
        ...
        raise RuntimeError("A426 A422 qualification semantics differ")
    return value


def freeze_implementation() -> dict[str, Any]:
    if IMPLEMENTATION.exists() or not no_execution_artifacts():
        raise FileExistsError("A426 implementation or downstream artifact already exists")
    design = load_design()
    _source, roles, orders, tasks = load_a416_schedule()
    if A422_QUALIFICATION.exists():
        raise RuntimeError("A426 implementation freeze must precede A422 qualification")
    if not TEST.exists() or not REPRO.exists():
        raise FileNotFoundError("A426 test and reproducer must precede implementation freeze")
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w52-a416-fresh-shared-stop-recovery-a426-implementation-v1",
        "attempt_id": ATTEMPT_ID,
        "commitment_state": "complete_A416_schedule_executor_and_fresh_W52_challenge_factory_frozen_before_any_A426_challenge_assignment_candidate_or_progress",
        ...
        "A426_challenge_or_assignment_available_at_freeze": False,
        "A426_candidate_or_progress_available_at_freeze": False,
...
def load_implementation(expected_sha256: str | None = None) -> dict[str, Any]:
    if expected_sha256 is not None and file_sha256(IMPLEMENTATION) != expected_sha256:
        raise RuntimeError("A426 implementation hash differs")
    value = json.loads(IMPLEMENTATION.read_bytes())
    if (
        value.get("schema")
        != "chacha20-round20-w52-a416-fresh-shared-stop-recovery-a426-implementation-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("commitment_state")
        != "complete_A416_schedule_executor_and_fresh_W52_challenge_factory_frozen_before_any_A426_challenge_assignment_candidate_or_progress"
        ...
        or value.get("A426_challenge_or_assignment_available_at_freeze") is not False
        or value.get("A426_candidate_or_progress_available_at_freeze") is not False
        or value.get("production_target_labels_used") != 0
        or value.get("production_reader_refits") != 0
        or value.get("prior_live_recovery_filter_outcomes_consumed") is not False
    ):
        raise RuntimeError("A426 implementation semantics differ")
    ...
    if value.get("implementation_commitment_sha256") != canonical_sha256(unsigned):
        raise RuntimeError("A426 implementation commitment differs")
    return value
...
def apply_assignment(
    ...
    if len(known_zeroed_key_words) != 8:
        raise ValueError("A426 requires eight ChaCha20 key words")
    if not 0 <= assignment < DOMAIN_SIZE:
        raise ValueError("A426 assignment exceeds W52")
    key = [int(word) & MASK32 for word in known_zeroed_key_words]
    if key[0] != 0 or key[1] & ((1 << WORD1_LOW_BITS) - 1):
        raise ValueError("A426 known key does not zero the W52 interval")
...
def challenge_from_assignment(*, label: str, assignment: int) -> dict[str, Any]:
    ...
    control[0] ^= 1
    return {
        "challenge_id": "chacha20-r20-w52-a426-fresh-a416-transfer-v1",
...
def validate_challenge(challenge: Mapping[str, Any]) -> None:
    if (
        ...
        != "chacha20-r20-w52-a426-fresh-a416-transfer-v1"
        ...
        != "key_word0_all32_plus_key_word1_low20"
        ...
        raise RuntimeError("A426 public challenge shape differs")
    ...
    if (
        ...
        raise RuntimeError("A426 public challenge identity differs")


def fresh_challenge() -> dict[str, Any]:
    label = f"A426|fresh-W52-A416-transfer|{secrets.token_hex(32)}"
...
def materialize_protocol(
    *,
    ...
    if not no_execution_artifacts():
        raise FileExistsError("A426 protocol or execution artifact already exists")
    if expected_a416_result_sha256 != A416_RESULT_SHA256:
        raise RuntimeError("A426 expected A416 result hash differs")
    implementation = load_implementation(expected_implementation_sha256)
    _source, roles, orders, tasks = load_a416_schedule()
    if A422_QUALIFICATION.exists():
        raise RuntimeError("A426 protocol must precede A422 qualification")
    ...
    if schedule_sha != implementation["source_A416_schedule_sha256"]:
        raise RuntimeError("A426 source schedule changed after implementation freeze")
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w52-a416-fresh-shared-stop-recovery-a426-protocol-v1",
        "attempt_id": ATTEMPT_ID,
        "protocol_state": "fresh_W52_challenge_and_byte_identical_A416_schedule_frozen_before_A422_qualification_any_A424_A425_outcome_or_any_A426_candidate_progress_or_filter_outcome",
        "design_sha256": DESIGN_SHA256,
        ...
        "information_boundary": {
            "A416_schedule_frozen_before_A426_challenge": True,
            "A422_qualification_available_at_A426_protocol_freeze": False,
            "A424_result_available_at_A426_protocol_freeze": False,
            "A425_result_available_at_A426_protocol_freeze": False,
            "A426_assignment_absent_from_protocol": True,
            "A426_candidate_or_progress_available_at_protocol_freeze": False,
            "A426_target_labels_used_for_schedule": 0,
            "A426_reader_refits": 0,
            "prior_live_recovery_filter_outcomes_consumed": False,
        },
        "anchors": {
            "design": anchor(DESIGN, DESIGN_SHA256),
...
def load_protocol(expected_sha256: str | None = None) -> dict[str, Any]:
    if expected_sha256 is not None and file_sha256(PROTOCOL) != expected_sha256:
        raise RuntimeError("A426 protocol hash differs")
    ...
    if (
        ...
        != "fresh_W52_challenge_and_byte_identical_A416_schedule_frozen_before_A422_qualification_any_A424_A425_outcome_or_any_A426_candidate_progress_or_filter_outcome"
        or value.get("design_sha256") != DESIGN_SHA256
        or value.get("source_A416_result_sha256") != A416_RESULT_SHA256
        or value.get("A422_protocol_sha256") != A422_PROTOCOL_SHA256
        or value.get("A422_qualification_available_at_protocol_freeze") is not False
        ...
        or boundary.get("A426_candidate_or_progress_available_at_protocol_freeze")
        is not False
        or boundary.get("A426_target_labels_used_for_schedule") != 0
        or boundary.get("A426_reader_refits") != 0
        or boundary.get("A422_qualification_available_at_A426_protocol_freeze") is not False
        or boundary.get("A424_result_available_at_A426_protocol_freeze") is not False
        or boundary.get("A425_result_available_at_A426_protocol_freeze") is not False
    ):
        raise RuntimeError("A426 protocol semantics differ")
    for row in value["anchors"].values():
        anchor(ROOT / row["path"], row["sha256"])
    validate_challenge(value["public_challenge"])
    if canonical_sha256(value["public_challenge"]) != value["public_challenge_sha256"]:
        raise RuntimeError("A426 public challenge commitment differs")
    _source, roles, source_orders, source_tasks = load_a416_schedule()
    if value["worker_roles"] != roles:
        raise RuntimeError("A426 worker role transfer differs")
    orders: dict[str, list[int]] = {}
    tasks: dict[str, list[dict[str, Any]]] = {}
    for index, role in enumerate(roles):
        orders[role], tasks[role] = validate_worker_schedule(
        ...
        if orders[role] != source_orders[role] or tasks[role] != source_tasks[role]:
            raise RuntimeError("A426 byte-identical A416 transfer differs")
        if worker_order_sha256(orders[role]) != value[
            "worker_order_uint16be_sha256"
        ][role]:
            raise RuntimeError("A426 worker order commitment differs")
        if task_list_sha256(tasks[role]) != value["worker_task_list_sha256"][role]:
            raise RuntimeError("A426 task commitment differs")
    schedule_sha = canonical_sha256(
        {"roles": roles, "orders": orders, "tasks": tasks}
    )
    if schedule_sha != value["schedule_sha256"]:
        raise RuntimeError("A426 schedule commitment differs")
    unsigned = {
        key: item
        for key, item in value.items()
    ...
    if value.get("protocol_commitment_sha256") != canonical_sha256(unsigned):
        raise RuntimeError("A426 protocol commitment differs")
...
def load_resume(
    *,
    ...
    if (
        value.get("schema")
        != "chacha20-round20-w52-a416-fresh-shared-stop-recovery-a426-progress-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        ...
        raise RuntimeError("A426 progress fingerprint differs")
    ...
    if (
        ...
        raise RuntimeError("A426 resumable progress state differs")
    ...
    try:
        for position in range(start_group, STATIC_EPOCHS):
            ...
            if position == start_group or position % HOST_REFRESH_GROUPS == 0:
                ...
                host = host_factory()
                host_instances += 1
            observed = A422.filter_complete_prefix(
                host=host,
                challenge=challenge,
                prefix=prefix,
                target=target,
            ...
            groups = position + 1
            if controls:
                raise RuntimeError("A426 matched control produced a candidate")
            common = {
            ...
            if len(factual) != 1:
                raise RuntimeError(
                    "A426 complete W52 group produced multiple factual filters"
                )
            candidate = factual[0]
            if ((candidate >> WORD0_SUFFIX_BITS) & (CELLS - 1)) != prefix:
                raise RuntimeError("A426 candidate prefix differs")
            ...
            found = {
                ...
                "complete_W52_group_execution_before_stop": True,
...
def mark_peer_confirmed(
    ...
    if (
        ...
        raise RuntimeError("A426 peer progress fingerprint differs")
    if value.get("status") not in {
        ...
        raise RuntimeError("A426 peer progress status differs")
...
def progress_snapshot(protocol: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for index, role in enumerate(roles):
        ...
        if (
            ...
            raise RuntimeError("A426 progress snapshot fingerprint differs")
...
def build_causal(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    terminal = (
        "confirmed_strict_subset_W52_recovery"
        if strict
        else "confirmed_complete_domain_W52_recovery"
    )
    writer = CausalWriter(api_id="a426w52")
    writer._rules = []
    writer.add_rule(
        name="qualified_A416_schedule_and_exact_W52_engine_to_model",
        description="Execute the byte-identical, externally qualified A416 eight-reader schedule as disjoint complete W52 groups with one confirmed shared stop.",
        pattern=["A426_frozen_fresh_W52_schedule", "A422_exact_W52_group_engine"],
        conclusion="A426_sole_factual_W52_model",
        confidence_modifier=1.0,
    )
    writer.add_rule(
        name="W52_model_to_confirmed_recovery",
        description="Require an empty matched control and independent eight-block confirmation before retaining the W52 recovery.",
        pattern=["A426_sole_factual_W52_model"],
        conclusion=f"A426_{terminal}",
        confidence_modifier=1.0,
    )
    writer.add_triplet(
        trigger="A426:frozen_fresh_W52_A416_schedule",
        mechanism="disjoint_complete_2^38_group_execution_with_confirmed_shared_stop",
        outcome="A426:sole_factual_W52_model",
        confidence=1.0,
        ...
        domain="full-round ChaCha20 W52 qualified Reader recovery",
        quality_score=1.0,
    )
    writer.add_triplet(
        trigger="A426:sole_factual_W52_model",
        mechanism="matched_control_rejection_and_dual_reference_eight_block_confirmation",
        outcome=f"A426:{terminal}",
        confidence=1.0,
    ...
    writer.add_triplet(
        trigger="A426:frozen_fresh_W52_A416_schedule",
        mechanism="materialized_A416_W52_search_and_confirmation_closure",
        outcome=f"A426:{terminal}",
        confidence=1.0,
        source="materialized:A426_A416_W52_recovery_chain",
    ...
    writer.add_cluster(
        name="A426 qualified A416 W52 recovery",
        entities=[
            "A426:frozen_fresh_W52_A416_schedule",
            "A426:sole_factual_W52_model",
            f"A426:{terminal}",
        ],
    )
    writer.add_gap(
        subject=f"A426:{terminal}",
        predicate="next_required_object",
        expected_object_type="fresh_W52_or_W52_byte_identical_A416_schedule_transfer",
        confidence=1.0,
        suggested_queries=[
            "Transfer the same frozen A416 schedule to the next exact grouped-engine width without using the A426 outcome for schedule selection."
        ],
    )
    ...
    if (
        ...
        raise RuntimeError("A426 authentic Causal reopen gate failed")
    return {
...
def finalize_confirmed_stop(
    ...
    if (
        stop.get("schema")
        != "chacha20-round20-w52-a416-fresh-shared-stop-recovery-a426-confirmed-stop-v1"
        ...
        raise RuntimeError("A426 confirmed stop fingerprint differs")
    ...
    if not 1 <= unique_groups <= CELLS:
        raise RuntimeError("A426 aggregate unique group count differs")
    ...
    evidence_stage = (
        "FULLROUND_R20_A416_READER_W52_STRICT_SUBSET_RECOVERY_CONFIRMED"
        if strict_subset
        else "FULLROUND_R20_A416_READER_W52_COMPLETE_DOMAIN_RECOVERY_CONFIRMED"
    )
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w52-a416-fresh-shared-stop-recovery-a426-result-v1",
        "attempt_id": ATTEMPT_ID,
        ...
        "anchors": {
            ...
            "A422_qualification": anchor(
                A422_QUALIFICATION, qualification_sha256
            ),
            "runner": anchor(Path(__file__)),
        },
    }
    ...
    atomic_bytes(
        REPORT,
        (
            "# A426 — Qualified A416 full-round ChaCha20 W52 recovery\n\n"
...
def recover_worker(
    *,
    ...
    expected_a422_qualification_sha256: str,
) -> dict[str, Any]:
    if not 0 <= worker_index < WORKERS:
        raise ValueError("A426 worker index differs")
    ...
    if STOP.exists():
        ...
        if int(stop.get("worker_index", -1)) == worker_index:
            return finalize_confirmed_stop(
                protocol, expected_a422_qualification_sha256
            )
        return mark_peer_confirmed(
            protocol, expected_a422_qualification_sha256, worker_index
        )
    roles = protocol["worker_roles"]
    ...
    def write_progress(row: Mapping[str, Any]) -> None:
        atomic_json(
            ...
            {
                "schema": "chacha20-round20-w52-a416-fresh-shared-stop-recovery-a426-progress-v1",
                "attempt_id": ATTEMPT_ID,
                "worker_index": worker_index,
                "worker_role": role,
    ...
    if discovery["status"] in {"peer_confirmed", "worker_exhausted"}:
        ...
        if discovery["status"] == "worker_exhausted":
            ...
            if exhausted == WORKERS:
                raise RuntimeError(
                    "A426 all disjoint workers exhausted without a model"
                )
        return discovery
    ...
    if confirmation.get("all_blocks_match") is not True:
        raise RuntimeError("A426 dual-reference eight-block confirmation failed")
    ...
    atomic_json(
        STOP,
        {
            "schema": "chacha20-round20-w52-a416-fresh-shared-stop-recovery-a426-confirmed-stop-v1",
            "attempt_id": ATTEMPT_ID,
            "protocol_sha256": expected_protocol_sha256,
            "A422_qualification_sha256": expected_a422_qualification_sha256,
            "worker_index": worker_index,
...
def main() -> None:
    ...
    if args.freeze_implementation:
        ...
        if (
            ...
            or not args.expected_a422_qualification_sha256
        ):
            parser.error(
                "--recover-worker requires protocol and A422 qualification hashes"
            )
