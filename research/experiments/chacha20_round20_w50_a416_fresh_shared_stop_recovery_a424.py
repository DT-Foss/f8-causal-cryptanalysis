#!/usr/bin/env python3
"""A424: execute the qualified A416 schedule on a fresh W50 challenge."""
...
STEM = "chacha20_round20_w50_a416_fresh_shared_stop_recovery_a424"
...
ATTEMPT_ID = "A424"
DESIGN_SHA256 = "658c9dc021b6629f99872302f744d4b24cf640ca0bb5617cd64713b8725744d4"
...
def load_module(path: Path, name: str) -> Any:
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot import A424 dependency {path}")
...
A384 = load_module(A384_RUNNER, "a424_a384")
W43 = A384.W43
...
def progress_path(worker_index: int) -> Path:
    if not 0 <= worker_index < WORKERS:
        raise ValueError("A424 worker index differs")
...
def load_design() -> dict[str, Any]:
    if file_sha256(DESIGN) != DESIGN_SHA256:
        raise RuntimeError("A424 design hash differs")
    ...
    if (
        value.get("schema")
        != "chacha20-round20-w50-a416-fresh-shared-stop-recovery-a424-design-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("design_state")
        != "frozen_after_A416_external_qualification_and_A384_target_free_engine_qualification_before_any_A424_challenge_assignment_candidate_progress_or_filter_outcome"
        ...
        or transfer.get("parameter_refit_on_A424_target") is not False
        or fresh.get("materialize_only_after_complete_A424_implementation_freeze") is not True
        or fresh.get("secret_assignment_absent_from_protocol") is not True
        or boundary.get("A416_result_available_at_A424_design_freeze") is not True
        or boundary.get("A384_qualification_available_at_A424_design_freeze") is not True
        or boundary.get("A424_production_challenge_available_at_design_freeze") is not False
        or boundary.get("A424_target_assignment_available_at_design_freeze") is not False
        or boundary.get("A424_candidate_or_progress_available_at_design_freeze") is not False
        or boundary.get("A424_target_labels_used_for_schedule") != 0
        or boundary.get("prior_live_recovery_filter_outcomes_consumed") is not False
    ):
        raise RuntimeError("A424 frozen design semantics differ")
...
def validate_worker_schedule(
    *,
    ...
    if not 0 <= worker_index < WORKERS or roles[worker_index] != worker_role:
        raise ValueError("A424 worker identity differs")
    ...
    if (
        ...
        raise ValueError("A424 worker schedule geometry differs")
    for step, (cell, row) in enumerate(zip(order, rows, strict=True), 1):
        ...
        if (
            ...
            raise ValueError(
                f"A424 task semantics differ at worker {worker_index} step {step}"
            )
    return order, rows
...
def load_a416_schedule() -> tuple[
    ...
    if file_sha256(A416_RESULT) != A416_RESULT_SHA256:
        raise RuntimeError("A424 A416 result hash differs")
    ...
    if (
        ...
        raise RuntimeError("A424 A416 source semantics differ")
    ...
    if len(flat) != CELLS or len(set(flat)) != CELLS or set(flat) != set(range(CELLS)):
        raise RuntimeError("A424 A416 schedule cover differs")
    return value, roles, orders, tasks


def load_a384_qualification(expected_sha256: str) -> dict[str, Any]:
    if expected_sha256 != A384_QUALIFICATION_SHA256:
        raise RuntimeError("A424 expected A384 qualification hash differs")
    if file_sha256(A384_QUALIFICATION) != expected_sha256:
        raise RuntimeError("A424 A384 qualification artifact hash differs")
    value = json.loads(A384_QUALIFICATION.read_bytes())
    ...
    if (
        ...
        raise RuntimeError("A424 A384 qualification semantics differ")
    return value


def freeze_implementation() -> dict[str, Any]:
    if IMPLEMENTATION.exists() or not no_execution_artifacts():
        raise FileExistsError("A424 implementation or downstream artifact already exists")
    design = load_design()
    _source, roles, orders, tasks = load_a416_schedule()
    qualification = load_a384_qualification(A384_QUALIFICATION_SHA256)
    if not TEST.exists() or not REPRO.exists():
        raise FileNotFoundError("A424 test and reproducer must precede implementation freeze")
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w50-a416-fresh-shared-stop-recovery-a424-implementation-v1",
        "attempt_id": ATTEMPT_ID,
        "commitment_state": "complete_A416_schedule_executor_and_fresh_W50_challenge_factory_frozen_before_any_A424_challenge_assignment_candidate_or_progress",
        ...
        "A384_engine_qualification_sha256": qualification["qualification_sha256"],
        "A424_challenge_or_assignment_available_at_freeze": False,
        "A424_candidate_or_progress_available_at_freeze": False,
        ...
        "anchors": {
            ...
            "A384_qualification": anchor(
                A384_QUALIFICATION, A384_QUALIFICATION_SHA256
            ),
...
def load_implementation(expected_sha256: str | None = None) -> dict[str, Any]:
    if expected_sha256 is not None and file_sha256(IMPLEMENTATION) != expected_sha256:
        raise RuntimeError("A424 implementation hash differs")
    value = json.loads(IMPLEMENTATION.read_bytes())
    if (
        value.get("schema")
        != "chacha20-round20-w50-a416-fresh-shared-stop-recovery-a424-implementation-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("commitment_state")
        != "complete_A416_schedule_executor_and_fresh_W50_challenge_factory_frozen_before_any_A424_challenge_assignment_candidate_or_progress"
        ...
        or value.get("A424_challenge_or_assignment_available_at_freeze") is not False
        or value.get("A424_candidate_or_progress_available_at_freeze") is not False
        or value.get("production_target_labels_used") != 0
        or value.get("production_reader_refits") != 0
        or value.get("prior_live_recovery_filter_outcomes_consumed") is not False
    ):
        raise RuntimeError("A424 implementation semantics differ")
    ...
    if value.get("implementation_commitment_sha256") != canonical_sha256(unsigned):
        raise RuntimeError("A424 implementation commitment differs")
    return value
...
def apply_assignment(
    ...
    if len(known_zeroed_key_words) != 8:
        raise ValueError("A424 requires eight ChaCha20 key words")
    if not 0 <= assignment < DOMAIN_SIZE:
        raise ValueError("A424 assignment exceeds W50")
    key = [int(word) & MASK32 for word in known_zeroed_key_words]
    if key[0] != 0 or key[1] & ((1 << WORD1_LOW_BITS) - 1):
        raise ValueError("A424 known key does not zero the W50 interval")
...
def challenge_from_assignment(*, label: str, assignment: int) -> dict[str, Any]:
    ...
    control[0] ^= 1
    return {
        "challenge_id": "chacha20-r20-w50-a424-fresh-a416-transfer-v1",
...
def validate_challenge(challenge: Mapping[str, Any]) -> None:
    if (
        ...
        != "chacha20-r20-w50-a424-fresh-a416-transfer-v1"
        ...
        != "key_word0_all32_plus_key_word1_low18"
        ...
        raise RuntimeError("A424 public challenge shape differs")
    ...
    if (
        ...
        raise RuntimeError("A424 public challenge identity differs")


def fresh_challenge() -> dict[str, Any]:
    label = f"A424|fresh-W50-A416-transfer|{secrets.token_hex(32)}"
...
def materialize_protocol(
    *,
    ...
    expected_a384_qualification_sha256: str,
) -> dict[str, Any]:
    if not no_execution_artifacts():
        raise FileExistsError("A424 protocol or execution artifact already exists")
    if expected_a416_result_sha256 != A416_RESULT_SHA256:
        raise RuntimeError("A424 expected A416 result hash differs")
    ...
    if schedule_sha != implementation["source_A416_schedule_sha256"]:
        raise RuntimeError("A424 source schedule changed after implementation freeze")
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w50-a416-fresh-shared-stop-recovery-a424-protocol-v1",
        "attempt_id": ATTEMPT_ID,
        "protocol_state": "fresh_W50_challenge_and_byte_identical_A416_schedule_frozen_before_any_A424_candidate_progress_or_filter_outcome",
        "design_sha256": DESIGN_SHA256,
        ...
        "A384_protocol_sha256": A384_PROTOCOL_SHA256,
        "A384_qualification_sha256": expected_a384_qualification_sha256,
        "A384_engine_qualification_sha256": qualification["qualification_sha256"],
        "worker_roles": roles,
        ...
        "information_boundary": {
            "A416_schedule_frozen_before_A424_challenge": True,
            "A384_engine_qualified_before_A424_challenge": True,
            "A424_assignment_absent_from_protocol": True,
            "A424_candidate_or_progress_available_at_protocol_freeze": False,
            "A424_target_labels_used_for_schedule": 0,
            "A424_reader_refits": 0,
            "prior_live_recovery_filter_outcomes_consumed": False,
        },
        "anchors": {
            "design": anchor(DESIGN, DESIGN_SHA256),
            ...
            "A384_protocol": anchor(A384_PROTOCOL, A384_PROTOCOL_SHA256),
            "A384_qualification": anchor(
                A384_QUALIFICATION, expected_a384_qualification_sha256
            ),
            "runner": anchor(Path(__file__)),
        },
    }
...
def load_protocol(expected_sha256: str | None = None) -> dict[str, Any]:
    if expected_sha256 is not None and file_sha256(PROTOCOL) != expected_sha256:
        raise RuntimeError("A424 protocol hash differs")
    value = json.loads(PROTOCOL.read_bytes())
    boundary = value.get("information_boundary", {})
    if (
        value.get("schema")
        != "chacha20-round20-w50-a416-fresh-shared-stop-recovery-a424-protocol-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("protocol_state")
        != "fresh_W50_challenge_and_byte_identical_A416_schedule_frozen_before_any_A424_candidate_progress_or_filter_outcome"
        or value.get("design_sha256") != DESIGN_SHA256
        or value.get("source_A416_result_sha256") != A416_RESULT_SHA256
        or value.get("A384_protocol_sha256") != A384_PROTOCOL_SHA256
        or value.get("A384_qualification_sha256") != A384_QUALIFICATION_SHA256
        ...
        or boundary.get("A424_assignment_absent_from_protocol") is not True
        or boundary.get("A424_candidate_or_progress_available_at_protocol_freeze")
        is not False
        or boundary.get("A424_target_labels_used_for_schedule") != 0
        or boundary.get("A424_reader_refits") != 0
    ):
        raise RuntimeError("A424 protocol semantics differ")
    for row in value["anchors"].values():
        anchor(ROOT / row["path"], row["sha256"])
    validate_challenge(value["public_challenge"])
    if canonical_sha256(value["public_challenge"]) != value["public_challenge_sha256"]:
        raise RuntimeError("A424 public challenge commitment differs")
    _source, roles, source_orders, source_tasks = load_a416_schedule()
    if value["worker_roles"] != roles:
        raise RuntimeError("A424 worker role transfer differs")
    orders: dict[str, list[int]] = {}
    tasks: dict[str, list[dict[str, Any]]] = {}
    for index, role in enumerate(roles):
        orders[role], tasks[role] = validate_worker_schedule(
        ...
        if orders[role] != source_orders[role] or tasks[role] != source_tasks[role]:
            raise RuntimeError("A424 byte-identical A416 transfer differs")
        if worker_order_sha256(orders[role]) != value[
            "worker_order_uint16be_sha256"
        ][role]:
            raise RuntimeError("A424 worker order commitment differs")
        if task_list_sha256(tasks[role]) != value["worker_task_list_sha256"][role]:
            raise RuntimeError("A424 task commitment differs")
    schedule_sha = canonical_sha256(
        {"roles": roles, "orders": orders, "tasks": tasks}
    )
    if schedule_sha != value["schedule_sha256"]:
        raise RuntimeError("A424 schedule commitment differs")
    unsigned = {
        key: item
        for key, item in value.items()
    ...
    if value.get("protocol_commitment_sha256") != canonical_sha256(unsigned):
        raise RuntimeError("A424 protocol commitment differs")
    load_implementation(value["implementation_sha256"])
    load_a384_qualification(value["A384_qualification_sha256"])
    return value
...
def load_resume(
    *,
    ...
    if (
        value.get("schema")
        != "chacha20-round20-w50-a416-fresh-shared-stop-recovery-a424-progress-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        ...
        raise RuntimeError("A424 progress fingerprint differs")
    ...
    if (
        ...
        raise RuntimeError("A424 resumable progress state differs")
    ...
    *,
    ...
    try:
        for position in range(start_group, STATIC_EPOCHS):
            ...
            groups = position + 1
            if controls:
                raise RuntimeError("A424 matched control produced a candidate")
            common = {
            ...
            if len(factual) != 1:
                raise RuntimeError(
                    "A424 complete W50 group produced multiple factual filters"
                )
            candidate = factual[0]
            if ((candidate >> WORD0_SUFFIX_BITS) & (CELLS - 1)) != prefix:
                raise RuntimeError("A424 candidate prefix differs")
...
def mark_peer_confirmed(
    ...
    if (
        ...
        raise RuntimeError("A424 peer progress fingerprint differs")
    if value.get("status") not in {
        ...
        raise RuntimeError("A424 peer progress status differs")
...
def progress_snapshot(protocol: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for index, role in enumerate(roles):
        ...
        if (
            ...
            raise RuntimeError("A424 progress snapshot fingerprint differs")
...
def build_causal(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    terminal = (
        "confirmed_strict_subset_W50_recovery"
        if strict
        else "confirmed_complete_domain_W50_recovery"
    )
    writer = CausalWriter(api_id="a424w50")
    writer._rules = []
    writer.add_rule(
        name="qualified_A416_schedule_and_exact_W50_engine_to_model",
        description="Execute the byte-identical, externally qualified A416 eight-reader schedule as disjoint complete W50 groups with one confirmed shared stop.",
        pattern=["A424_frozen_fresh_W50_schedule", "A384_exact_W50_group_engine"],
        conclusion="A424_sole_factual_W50_model",
        confidence_modifier=1.0,
    )
    writer.add_rule(
        name="W50_model_to_confirmed_recovery",
        description="Require an empty matched control and independent eight-block confirmation before retaining the W50 recovery.",
        pattern=["A424_sole_factual_W50_model"],
        conclusion=f"A424_{terminal}",
        confidence_modifier=1.0,
    )
    writer.add_triplet(
        trigger="A424:frozen_fresh_W50_A416_schedule",
        mechanism="disjoint_complete_2^38_group_execution_with_confirmed_shared_stop",
        outcome="A424:sole_factual_W50_model",
        confidence=1.0,
        ...
        domain="full-round ChaCha20 W50 qualified Reader recovery",
        quality_score=1.0,
    )
    writer.add_triplet(
        trigger="A424:sole_factual_W50_model",
        mechanism="matched_control_rejection_and_dual_reference_eight_block_confirmation",
        outcome=f"A424:{terminal}",
        confidence=1.0,
    ...
    writer.add_triplet(
        trigger="A424:frozen_fresh_W50_A416_schedule",
        mechanism="materialized_A416_W50_search_and_confirmation_closure",
        outcome=f"A424:{terminal}",
        confidence=1.0,
        source="materialized:A424_A416_W50_recovery_chain",
    ...
    writer.add_cluster(
        name="A424 qualified A416 W50 recovery",
        entities=[
            "A424:frozen_fresh_W50_A416_schedule",
            "A424:sole_factual_W50_model",
            f"A424:{terminal}",
        ],
    )
    writer.add_gap(
        subject=f"A424:{terminal}",
        predicate="next_required_object",
        expected_object_type="fresh_W51_or_W52_byte_identical_A416_schedule_transfer",
        confidence=1.0,
        suggested_queries=[
            "Transfer the same frozen A416 schedule to the next exact grouped-engine width without using the A424 outcome for schedule selection."
        ],
    )
    ...
    if (
        ...
        raise RuntimeError("A424 authentic Causal reopen gate failed")
    return {
...
def finalize_confirmed_stop(
    ...
    if (
        stop.get("schema")
        != "chacha20-round20-w50-a416-fresh-shared-stop-recovery-a424-confirmed-stop-v1"
        or stop.get("attempt_id") != ATTEMPT_ID
        or stop.get("protocol_sha256") != file_sha256(PROTOCOL)
        or stop.get("A384_qualification_sha256") != qualification_sha256
        or stop.get("confirmation", {}).get("all_blocks_match") is not True
        or stop.get("discovery", {}).get("control_filter_candidates") != []
    ):
        raise RuntimeError("A424 confirmed stop fingerprint differs")
    ...
    if not 1 <= unique_groups <= CELLS:
        raise RuntimeError("A424 aggregate unique group count differs")
    ...
    evidence_stage = (
        "FULLROUND_R20_A416_READER_W50_STRICT_SUBSET_RECOVERY_CONFIRMED"
        if strict_subset
        else "FULLROUND_R20_A416_READER_W50_COMPLETE_DOMAIN_RECOVERY_CONFIRMED"
    )
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w50-a416-fresh-shared-stop-recovery-a424-result-v1",
        "attempt_id": ATTEMPT_ID,
        ...
        "A384_qualification_sha256": qualification_sha256,
        "discovery": discovery,
        ...
        "anchors": {
            ...
            "A384_qualification": anchor(
                A384_QUALIFICATION, qualification_sha256
            ),
            "runner": anchor(Path(__file__)),
        },
    }
    ...
    atomic_bytes(
        REPORT,
        (
            "# A424 — Qualified A416 full-round ChaCha20 W50 recovery\n\n"
            f"Evidence stage: **{evidence_stage}**\n\n"
            "- Frozen external scheduler: **A416**\n"
            f"- Exact unique W50 groups evaluated: **{unique_groups} / 4096**\n"
...
def recover_worker(
    *,
    ...
    load_a384_qualification(expected_a384_qualification_sha256)
    if RESULT.exists():
        return {"status": "peer_confirmed", "result_sha256": file_sha256(RESULT)}
    if STOP.exists():
        stop = json.loads(STOP.read_bytes())
        if int(stop.get("worker_index", -1)) == worker_index:
            return finalize_confirmed_stop(
                protocol, expected_a384_qualification_sha256
            )
        return mark_peer_confirmed(
            protocol, expected_a384_qualification_sha256, worker_index
        )
    roles = protocol["worker_roles"]
    ...
    def write_progress(row: Mapping[str, Any]) -> None:
        atomic_json(
            ...
            {
                "schema": "chacha20-round20-w50-a416-fresh-shared-stop-recovery-a424-progress-v1",
                "attempt_id": ATTEMPT_ID,
                "worker_index": worker_index,
                "worker_role": role,
    ...
    start, prior_gpu, prior_hosts, completed = load_resume(
        worker_index=worker_index,
        protocol_sha256=expected_protocol_sha256,
        qualification_sha256=expected_a384_qualification_sha256,
        order_sha=order_sha,
        tasks_sha=tasks_sha,
    )
    if completed is not None:
        if completed.get("status") == "candidate_found" and not STOP.exists():
            ...
            return recover_worker(
                ...
                expected_a384_qualification_sha256=expected_a384_qualification_sha256,
            )
        else:
            return completed
    else:
    ...
    if discovery["status"] in {"peer_confirmed", "worker_exhausted"}:
        ...
        if discovery["status"] == "worker_exhausted":
            ...
            if exhausted == WORKERS:
                raise RuntimeError(
                    "A424 all disjoint workers exhausted without a model"
                )
        return discovery
    ...
    if confirmation.get("all_blocks_match") is not True:
        raise RuntimeError("A424 dual-reference eight-block confirmation failed")
    ...
    atomic_json(
        STOP,
        {
            "schema": "chacha20-round20-w50-a416-fresh-shared-stop-recovery-a424-confirmed-stop-v1",
            "attempt_id": ATTEMPT_ID,
            "protocol_sha256": expected_protocol_sha256,
            "A384_qualification_sha256": expected_a384_qualification_sha256,
            "worker_index": worker_index,
    ...
    return finalize_confirmed_stop(protocol, expected_a384_qualification_sha256)
...
def analyze() -> dict[str, Any]:
    payload: dict[str, Any] = {
        ...
        "A384_qualification_complete": A384_QUALIFICATION.exists(),
    ...
    if A384_QUALIFICATION.exists():
        payload["A384_qualification_sha256"] = file_sha256(A384_QUALIFICATION)
        load_a384_qualification(payload["A384_qualification_sha256"])
    if PROTOCOL.exists():
...
def main() -> None:
    ...
    if args.freeze_implementation:
        ...
        if (
            ...
            or not args.expected_a384_qualification_sha256
        ):
            parser.error(
                "--materialize-protocol requires implementation, A416 result and A384 qualification hashes"
            )
        ...
        if (
            ...
            parser.error(
                "--recover-worker requires protocol and A384 qualification hashes"
            )
