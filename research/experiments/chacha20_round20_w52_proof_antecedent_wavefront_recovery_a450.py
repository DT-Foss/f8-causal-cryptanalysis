#!/usr/bin/env python3
"""A450: execute A449's target-blind proof-antecedent W52 wavefront."""
...
STEM = "chacha20_round20_w52_proof_antecedent_wavefront_recovery_a450"
...
ATTEMPT_ID = "A450"
DESIGN_SHA256 = "1046448498ff94486c0b79ccdea7d568dae0c59b38ecfcaee81cdbc0d0953210"
...
A449_PAIR_STREAM_SHA256 = "b54b08eb03164e6f15ddbb9c1709780c435915b0aa65e65154f92b469192c637"
A449_PREFIX_ORDER_SHA256 = "988ebd96289759f711ce28d664c0436c116590d8adb3302f5f4b808547c0883d"
A449_OFF_AXIS_ORDER_SHA256 = "203167494cfb192ab0450c64a6101ff98f59e6e15b4c415850467a57046f1e2a"
...
def load_module(path: Path, name: str) -> Any:
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot import A450 dependency {path}")
...
A449 = load_module(A449_RUNNER, "a450_a449")
...
order_sha256 = A449.A442.order_sha256
_A435: Any | None = None
...
def runtime_a435() -> Any:
    ...
    if _A435 is None:
        ...
        _A435 = load_module(A435_RUNNER, "a450_a435_runtime")
    return _A435


def progress_path(worker_index: int) -> Path:
    if not 0 <= worker_index < WORKERS:
        raise ValueError("A450 worker index differs")
...
def load_design() -> dict[str, Any]:
    ...
    if (
        value.get("schema")
        != "chacha20-round20-w52-proof-antecedent-wavefront-recovery-a450-design-v1"
        ...
        or schedule.get("source_attempt") != "A449"
        or schedule.get("source_selected_operator") != "proof_borda_top32"
        or schedule.get("source_result_commitment_sha256")
        != A449_RESULT_COMMITMENT_SHA256
        or schedule.get("source_pair_stream_uint16be_uint16be_sha256")
        != A449_PAIR_STREAM_SHA256
        or schedule.get("prefix_axis_order_sha256") != A449_PREFIX_ORDER_SHA256
        or schedule.get("off_axis_order_sha256") != A449_OFF_AXIS_ORDER_SHA256
        or schedule.get("A448_calibration_positive_fixed_blocks") != 8
        ...
        or engine.get("A449_proof_borda_top32_pair_order_used") is not True
        ...
        or queue.get("A450_runs_after_terminal_A440") is not True
        or queue.get("A450_runs_after_A440_supervisor_and_workers_close") is not True
        or boundary.get(
            "A449_result_and_authentic_Causal_personally_read_at_design_freeze"
        )
        is not True
        or boundary.get("A434_qualification_content_read_for_schedule") is not False
        or boundary.get("A450_candidate_or_progress_available_at_design_freeze")
        is not False
        or boundary.get("A450_target_labels_used_for_schedule") != 0
        or boundary.get("A450_feature_refits") != 0
        or boundary.get("A450_model_refits") != 0
        or boundary.get("prior_live_recovery_filter_outcomes_consumed") is not False
    ):
        raise RuntimeError("A450 frozen design semantics differ")
...
def load_source_schedule() -> tuple[dict[str, Any], list[int], list[int]]:
    ...
    packaged = A449.load_result(A449_RESULT_SHA256)
    schedule = packaged["recovery_schedule"]
    prefix = [int(value) for value in schedule["prefix_order"]]
    off_axis = [int(value) for value in schedule["off_axis_order"]]
    A449.exact_order(prefix, AXIS_CELLS)
    A449.exact_order(off_axis, AXIS_CELLS)
    if (
        packaged.get("schema")
        != "chacha20-round20-w52-target-blind-proof-antecedent-trace-a449-v1"
        or packaged.get("result_commitment_sha256")
        != A449_RESULT_COMMITMENT_SHA256
        or packaged.get("recovery_operator") != "proof_borda_top32"
        or packaged.get("recovery_ready") is not True
        or schedule.get("operator") != "proof_borda_top32"
        or schedule.get("pair_stream_uint16be_uint16be_sha256")
        != A449_PAIR_STREAM_SHA256
        or schedule.get("material_pair_diversity_vs_A442") is not True
        or packaged.get("W52_target_labels_used") != 0
        or packaged.get("W52_feature_refits") != 0
        or packaged.get("W52_model_refits") != 0
        or packaged.get("production_candidate_assignments_executed") != 0
        or packaged.get("A426_A438_A440_A443_secret_result_stop_or_worker_progress_read")
        is not False
        or order_sha256(prefix) != A449_PREFIX_ORDER_SHA256
        or order_sha256(off_axis) != A449_OFF_AXIS_ORDER_SHA256
        or len(prefix) * len(off_axis) != PAIR_CELLS
    ):
        raise RuntimeError("A450 A449 source schedule differs")
    source = dict(schedule)
    source.update(
        {
            "schema": packaged["schema"],
            "result_commitment_sha256": packaged["result_commitment_sha256"],
            "selected_operator": packaged["recovery_operator"],
            "material_pair_diversity": schedule["material_pair_diversity_vs_A442"],
            "target_labels_used_for_W52": packaged["W52_target_labels_used"],
            "feature_refits_on_W52": packaged["W52_feature_refits"],
            "model_refits_on_W52": packaged["W52_model_refits"],
            "candidate_assignments_executed": packaged[
                "production_candidate_assignments_executed"
            ],
            "anchors": packaged["anchors"],
        }
    )
    return source, prefix, off_axis
...
def load_public_challenge() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("A450 A426 public challenge scope differs")
    return a426, challenge


def freeze_implementation() -> dict[str, Any]:
    if IMPLEMENTATION.exists() or not no_downstream_artifacts():
        raise FileExistsError("A450 implementation or downstream artifact exists")
    design = load_design()
    source, prefix, off_axis = load_source_schedule()
    a426, _challenge = load_public_challenge()
    if not TEST.exists() or not REPRO.exists():
        raise FileNotFoundError("A450 tests and reproducer must precede freeze")
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w52-proof-antecedent-wavefront-recovery-a450-implementation-v1",
        "attempt_id": ATTEMPT_ID,
        "commitment_state": "complete_restart_safe_eight_worker_A449_proof_borda_top32_executor_frozen_without_reading_any_prior_secret_result_stop_or_worker_progress",
        ...
        "source_A449_pair_stream_sha256": source[
            "pair_stream_uint16be_uint16be_sha256"
        ],
        "source_A449_prefix_order_sha256": source["prefix_order_uint16be_sha256"],
        "source_A449_off_axis_order_sha256": source[
        ...
        "A450_candidate_or_progress_available_at_freeze": False,
...
def load_implementation(expected_sha256: str) -> dict[str, Any]:
    ...
    if (
        value.get("schema")
        != "chacha20-round20-w52-proof-antecedent-wavefront-recovery-a450-implementation-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("design_sha256") != DESIGN_SHA256
        or value.get("source_A449_result_sha256") != A449_RESULT_SHA256
        or value.get("source_A449_result_commitment_sha256")
        != A449_RESULT_COMMITMENT_SHA256
        or value.get("source_A449_pair_stream_sha256")
        != A449_PAIR_STREAM_SHA256
        or value.get("source_A449_prefix_order_sha256")
        != A449_PREFIX_ORDER_SHA256
        or value.get("source_A449_off_axis_order_sha256")
        != A449_OFF_AXIS_ORDER_SHA256
        ...
        is not False
        or value.get("A450_candidate_or_progress_available_at_freeze") is not False
        ...
        raise RuntimeError("A450 implementation semantics differ")
    ...
    if value.get("implementation_commitment_sha256") != canonical_sha256(unsigned):
        raise RuntimeError("A450 implementation commitment differs")
    return value
...
def square_pair_at(
    ...
    if not 0 <= global_index < PAIR_CELLS:
        raise ValueError("A450 global pair index differs")
    prefix = A449.exact_order(prefix_order, AXIS_CELLS)
    off_axis = A449.exact_order(off_axis_order, AXIS_CELLS)
...
def pair_global_index(
    ...
    if not 0 <= prefix < AXIS_CELLS or not 0 <= off_axis < AXIS_CELLS:
        raise ValueError("A450 pair coordinate differs")
    ...
    if square_pair_at(index, prefix_values, off_values) != (prefix, off_axis):
        raise RuntimeError("A450 square-wavefront inverse identity failed")
    return index


def freeze_protocol(*, expected_implementation_sha256: str) -> dict[str, Any]:
    if not no_downstream_artifacts():
        raise FileExistsError("A450 protocol or execution artifact exists")
    ...
    schedule = {
        "algorithm": "A449_target_blind_proof_antecedent_borda_top32_dual_axis_square_wavefront",
        "selected_proof_operator": "proof_borda_top32",
        "pair_stream_uint16be_uint16be_sha256": A449_PAIR_STREAM_SHA256,
        "A449_result_commitment_sha256": A449_RESULT_COMMITMENT_SHA256,
        "prefix_order_uint16be_sha256": A449_PREFIX_ORDER_SHA256,
        "off_axis_order_uint16be_sha256": A449_OFF_AXIS_ORDER_SHA256,
        "A448_calibration_minimum_fixed_block_bit_gain": source[
            "A448_calibration"
        ][
            "minimum_fixed_block_bit_gain"
        ],
        "A448_calibration_balanced_eight_block_bit_gain": source[
            "A448_calibration"
        ][
            "balanced_eight_block_bit_gain"
        ],
        "pair_spearman_to_A442": source["pair_comparison_to_A442"][
            "spearman_rank_correlation"
        ],
        "top65536_intersection_with_A442": source["pair_comparison_to_A442"][
            "top_k_overlap"
        ]["65536"]["intersection"],
        "top65536_displaced_cells_vs_A442": 65536
        - source["pair_comparison_to_A442"]["top_k_overlap"]["65536"][
            "intersection"
        ],
        ...
        "worker_role_formula": "proof_antecedent_wave_{worker_index}",
        "pair_at_formula": "square_pair_at(global_index,A449_prefix_order,A449_off_axis_order)",
    ...
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w52-proof-antecedent-wavefront-recovery-a450-protocol-v1",
        "attempt_id": ATTEMPT_ID,
        "protocol_state": "A449_exact_proof_borda_top32_square_schedule_and_A426_public_challenge_bound_without_reading_any_prior_secret_result_stop_or_worker_progress",
        "design_sha256": DESIGN_SHA256,
        "implementation_sha256": expected_implementation_sha256,
        "implementation_commitment_sha256": implementation[
            "implementation_commitment_sha256"
        ],
        "A449_result_sha256": A449_RESULT_SHA256,
        "A449_result_commitment_sha256": A449_RESULT_COMMITMENT_SHA256,
        ...
        "worker_roles": [f"proof_antecedent_wave_{index}" for index in range(WORKERS)],
        "information_boundary": {
            "A449_schedule_frozen_before_A450_challenge_binding": True,
            "A434_qualification_content_read_at_A450_protocol_freeze": False,
            "A426_assignment_absent_from_protocol": True,
            "A426_A438_A440_A443_secret_result_stop_or_worker_progress_read": False,
            "A450_candidate_or_progress_available_at_protocol_freeze": False,
            "A450_target_labels_used_for_schedule": 0,
            "A450_feature_refits": 0,
            "A450_model_refits": 0,
            "prior_live_recovery_filter_outcomes_consumed": False,
        },
...
def load_protocol(expected_sha256: str) -> dict[str, Any]:
    ...
    if (
        value.get("schema")
        != "chacha20-round20-w52-proof-antecedent-wavefront-recovery-a450-protocol-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("design_sha256") != DESIGN_SHA256
        or value.get("A449_result_sha256") != A449_RESULT_SHA256
        or value.get("A449_result_commitment_sha256")
        != A449_RESULT_COMMITMENT_SHA256
        or value.get("A426_protocol_sha256") != A426_PROTOCOL_SHA256
        or schedule.get("algorithm")
        != "A449_target_blind_proof_antecedent_borda_top32_dual_axis_square_wavefront"
        or schedule.get("selected_proof_operator") != "proof_borda_top32"
        or schedule.get("pair_stream_uint16be_uint16be_sha256")
        != A449_PAIR_STREAM_SHA256
        or schedule.get("A449_result_commitment_sha256")
        != A449_RESULT_COMMITMENT_SHA256
        ...
        != [f"proof_antecedent_wave_{index}" for index in range(WORKERS)]
        or boundary.get("A449_schedule_frozen_before_A450_challenge_binding")
        is not True
        or boundary.get("A434_qualification_content_read_at_A450_protocol_freeze")
        is not False
        ...
        is not False
        or boundary.get("A450_candidate_or_progress_available_at_protocol_freeze")
        is not False
        or boundary.get("A450_target_labels_used_for_schedule") != 0
        or boundary.get("A450_feature_refits") != 0
        or boundary.get("A450_model_refits") != 0
        ...
        raise RuntimeError("A450 protocol semantics differ")
    for row in value["anchors"].values():
        anchor(path_from_ref(row["path"]), row["sha256"])
    ...
    if value.get("protocol_commitment_sha256") != canonical_sha256(unsigned):
        raise RuntimeError("A450 protocol commitment differs")
    return value
...
def worker_global_index(worker_index: int, zero_based_worker_step: int) -> int:
    ...
    if not 0 <= zero_based_worker_step < WORKER_TASKS:
        raise ValueError("A450 worker step differs")
...
def normalize_worker_row(row: Mapping[str, Any], worker_index: int) -> dict[str, Any]:
    value = dict(row)
    value["worker_role"] = f"proof_antecedent_wave_{worker_index}"
    return value
...
def load_resume(
    *, worker_index: int, protocol_sha256: str, qualification_sha256: str
    ...
    if (
        value.get("schema")
        != "chacha20-round20-w52-proof-antecedent-wavefront-recovery-a450-progress-v1"
        or value.get("attempt_id") != ATTEMPT_ID
        or value.get("worker_index") != worker_index
        or value.get("worker_role") != f"proof_antecedent_wave_{worker_index}"
        ...
        raise RuntimeError("A450 progress fingerprint differs")
    ...
    if (
        ...
        raise RuntimeError("A450 resumable progress state differs")
...
def mark_peer_confirmed(
    ...
    if (
        ...
        raise RuntimeError("A450 peer progress fingerprint differs")
    if value.get("status") in {"running", "peer_confirmed"}:
        ...
        raise RuntimeError("A450 peer progress status differs")
...
def progress_snapshot(protocol_sha256: str, qualification_sha256: str) -> dict[str, Any]:
    ...
    for index in range(WORKERS):
        role = f"proof_antecedent_wave_{index}"
        ...
        if (
            ...
            raise RuntimeError("A450 progress snapshot fingerprint differs")
...
def build_causal(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    writer = CausalWriter(api_id="a450recover")
    writer._rules = []
    writer.add_rule(
        name="proof_antecedent_schedule_to_factual_model",
        description="Execute the exact A449 proof-Borda-top32 pair order as eight disjoint complete-cell streams.",
        pattern=["A449_qualified_W52_proof_portfolio_recovery_execution"],
        conclusion="A450_sole_factual_W52_model",
    ...
    writer.add_rule(
        ...
        pattern=["A450_sole_factual_W52_model"],
        conclusion=f"A450_{terminal}",
        confidence_modifier=1.0,
    )
    writer.add_triplet(
        trigger="A449:qualified_W52_proof_portfolio_recovery_execution",
        mechanism="eight_disjoint_complete_cell_proof_borda_top32_wavefront_execution",
        outcome="A450:sole_factual_W52_model",
        confidence=1.0,
    ...
    writer.add_triplet(
        ...
        outcome=f"A450:{terminal}",
        confidence=1.0,
        source="materialized:A450_proof_antecedent_W52_recovery_chain",
    ...
    writer.add_cluster(
        name="A450 proof-antecedent Reader W52 recovery",
        entities=[
            "A449:qualified_W52_proof_portfolio_recovery_execution",
            "A450:sole_factual_W52_model",
            f"A450:{terminal}",
        ],
    )
    writer.add_gap(
        subject=f"A450:{terminal}",
        predicate="next_required_object",
        expected_object_type="fresh_cross_target_replication_or_W53_transfer",
        confidence=1.0,
        suggested_queries=[
            "Use the post-confirmation proof-Borda, A442, and A439 ranks to choose fresh W52 replication or W53 expansion."
        ],
    )
    ...
    if (
        ...
        raise RuntimeError("A450 authentic Causal reopen gate failed")
    return {
...
def finalize_confirmed_stop(
    ...
    if (
        stop.get("schema")
        != "chacha20-round20-w52-proof-antecedent-wavefront-recovery-a450-confirmed-stop-v1"
    ...
    if not 1 <= unique_cells <= PAIR_CELLS:
        raise RuntimeError("A450 aggregate unique pair-cell count differs")
    ...
    if (
        ...
        raise RuntimeError("A450 discovery rank identity differs")
    ...
    a442_row = source["anchors"]["A442_result"]
    a442_path = path_from_ref(a442_row["path"])
    anchor(a442_path, a442_row["sha256"])
    a442 = json.loads(a442_path.read_bytes())
    a442_prefix = [int(value) for value in a442["prefix_order"]]
    a442_off_axis = [int(value) for value in a442["off_axis_order"]]
    a442_global_index = pair_global_index(
        prefix, off_axis, a442_prefix, a442_off_axis
    )
    rank_analysis = {
        "prefix12": prefix,
        "off_axis12": off_axis,
        "proof_borda_top32_prefix_rank_one_based": prefix_rank,
        "proof_borda_top32_off_axis_rank_one_based": off_axis_rank,
        "proof_borda_top32_global_pair_rank_one_based": global_index + 1,
        "A442_global_pair_rank_one_based": a442_global_index + 1,
        "A439_global_pair_rank_one_based": a439_global_index + 1,
        "A442_over_proof_borda_rank_ratio": (a442_global_index + 1)
        / (global_index + 1),
        "proof_borda_gain_over_A442_bits": math.log2(
            (a442_global_index + 1) / (global_index + 1)
        ),
        "A439_over_proof_borda_rank_ratio": (a439_global_index + 1)
        / (global_index + 1),
        "proof_borda_gain_over_A439_bits": math.log2(
    ...
    evidence_stage = (
        "FULLROUND_R20_PROOF_ANTECEDENT_W52_STRICT_SUBSET_RECOVERY_CONFIRMED"
        if strict_subset
        else "FULLROUND_R20_PROOF_ANTECEDENT_W52_COMPLETE_DOMAIN_RECOVERY_CONFIRMED"
    )
    payload: dict[str, Any] = {
        "schema": "chacha20-round20-w52-proof-antecedent-wavefront-recovery-a450-result-v1",
        "attempt_id": ATTEMPT_ID,
        ...
        "A449_result_sha256": A449_RESULT_SHA256,
        "A449_result_commitment_sha256": A449_RESULT_COMMITMENT_SHA256,
        "A449_pair_stream_sha256": A449_PAIR_STREAM_SHA256,
        ...
        "anchors": {
            ...
            "A442_result": anchor(a442_path, a442_row["sha256"]),
    ...
    atomic_bytes(
        REPORT,
        (
            "# A450 — proof-antecedent Reader full-round ChaCha20 W52 recovery\n\n"
            f"Evidence stage: **{evidence_stage}**\n\n"
            "- Frozen schedule: **A449 target-blind proof-Borda-top32 dual-axis wavefront**\n"
            ...
            f"- Proof-Borda prefix / off-axis ranks: **{prefix_rank} / {off_axis_rank}**\n"
            f"- Proof-Borda global pair rank: **{global_index + 1:,}**\n"
            f"- A442 global pair rank: **{a442_global_index + 1:,}**\n"
            f"- A439 global pair rank: **{a439_global_index + 1:,}**\n"
            f"- Proof-Borda gain over A442: **{rank_analysis['proof_borda_gain_over_A442_bits']:.9f} bits**\n"
            f"- Proof-Borda gain over A439: **{rank_analysis['proof_borda_gain_over_A439_bits']:.9f} bits**\n"
...
def recover_worker(
    *,
    ...
    role = f"proof_antecedent_wave_{worker_index}"
    ...
    def write_progress(row: Mapping[str, Any]) -> None:
        ...
        atomic_json(
            ...
            {
                "schema": "chacha20-round20-w52-proof-antecedent-wavefront-recovery-a450-progress-v1",
                "attempt_id": ATTEMPT_ID,
                "worker_index": worker_index,
    ...
    if discovery["status"] in {"peer_confirmed", "worker_exhausted"}:
        ...
        if discovery["status"] == "worker_exhausted":
            ...
            if exhausted == WORKERS:
                raise RuntimeError("A450 all disjoint workers exhausted without a model")
        return discovery
    ...
    if confirmation.get("all_blocks_match") is not True:
        raise RuntimeError("A450 dual-reference eight-block confirmation failed")
    if discovery.get("control_filter_candidates") != []:
        raise RuntimeError("A450 matched control produced a candidate")
    if STOP.exists():
        return mark_peer_confirmed(
            expected_protocol_sha256,
            expected_a434_qualification_sha256,
    ...
    atomic_json(
        STOP,
        {
            "schema": "chacha20-round20-w52-proof-antecedent-wavefront-recovery-a450-confirmed-stop-v1",
            "attempt_id": ATTEMPT_ID,
            "protocol_sha256": expected_protocol_sha256,
            "A434_qualification_sha256": expected_a434_qualification_sha256,
            "A449_result_commitment_sha256": A449_RESULT_COMMITMENT_SHA256,
